...
export function useApi() {
  const { user } = useAuth()
  const { lowStockThreshold } = useAppStore()
  const uid = user?.id || ''

  // Fix #8 & #9: include threshold in headers so API uses user-defined value
  const headers = () => ({
    'Content-Type': 'application/json',
    'x-user-id': uid,
    'x-low-stock-threshold': String(lowStockThreshold || 10),
  })
  ...
  // Fix #11: returns inventoryUpdates for client-side state sync
  async function addDueEntry(data: Partial<DueLedgerEntry>): Promise<{
    entry: DueLedgerEntry;
    inventoryUpdates: { id: string; new_qty: number; status: string }[]
  }> {
    const res = await fetch('/api/due-ledger', { method: 'POST', headers: headers(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return { entry: d.data, inventoryUpdates: d.inventoryUpdates || [] }
  }
