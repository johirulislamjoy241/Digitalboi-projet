...
import type { InventoryItem, Transaction, Buyer } from '@/types'
import {
  Search, Plus, Minus, Trash2, ShoppingCart, Check, X,
  User, CreditCard, Banknote, Camera, FlipHorizontal, ScanLine, ChevronDown
} from 'lucide-react'
...
const CE: Record<string, string> = {
  ...
  Hardware: '🔧', Toys: '🎮', Sports: '⚽', Automotive: '🚗', Agriculture: '🌾', Other: '📋'
}
...
function CameraScanner({ onScan, onClose }: { onScan: (c: string) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setScanY(y => {
        const n = y + scanDir * 1.8
        if (n >= 90) { setScanDir(-1); return 90 }
        if (n <= 10) { setScanDir(1); return 10 }
        return n
      })
    }, 16)
      ...
      try {
        if (vRef.current.readyState >= 2) {
          ...
          if (r.length > 0 && r[0].rawValue) {
            done.current = true; stop(); onScan(r[0].rawValue.trim()); return
          }
        }
      } catch { /**/ }
      aRef.current = requestAnimationFrame(run)
    }
    aRef.current = requestAnimationFrame(run)
  }, [onScan, stop])
  ...
  const submit = () => { const c = manual.trim(); if (c) onScan(c) }
...
export default function POSSection() {
  const { inventory, setInventory, transactions, setTransactions, buyers, setBuyers, currency } = useAppStore()
  const api = useApi()
  ...
  // ✅ FIX 2: Buyer selection for due — searchable dropdown with full buyer list
  const [selBuyer, setSelBuyer] = useState<Buyer | null>(null)
  const [buyerSearch, setBuyerSearch] = useState('')
  const [showBuyerDD, setShowBuyerDD] = useState(false)
  const [manualCustomer, setManualCustomer] = useState('') // for cash mode optional name

  useEffect(() => {
    if (inventory.length === 0) api.getInventory().then(setInventory)
    // ✅ FIX 2: buyers load করা হচ্ছে POS section-এ
    if (buyers.length === 0) api.getBuyers().then(setBuyers)
  }, []) // eslint-disable-line

  /* ✅ FIX 2: filtered buyers for dropdown */
  ...
  function onScan(code: string) {
    ...
    const found = inventory.find(i =>
      i.status !== 'Archived' && (
        ...
        i.id === code || i.name.toLowerCase() === q
      )
  ...
  function adjQty(id: string, d: number) {
  ...
  function setPrice(id: string, v: string) {
    setCart(prev => prev.map(c => c.product.id === id ? { ...c, unitPrice: parseFloat(v) || 0 } : c))
  }
  ...
  // ✅ FIX 2: customer name — due mode uses selBuyer.name, cash mode uses manualCustomer
  const effectiveCustomer = payMode === 'due'
    ? (selBuyer?.name || '')
    : manualCustomer

  async function checkout() {
    if (cart.length === 0) return toast('কার্ট খালি', 'er')
    if (payMode === 'due' && !selBuyer) return toast('বকেয়ার জন্য ক্রেতা নির্বাচন করুন', 'er')
    setProcessing(true)
    try {
      let inv = [...inventory]
      ...
      for (const item of cart) {
        const res = await api.doTransaction({
          ...
          txn_type: 'out', quantity: item.qty, unit: item.product.unit,
          buy_price: item.product.buy_price, sell_price: item.unitPrice,
          notes: effectiveCustomer ? `POS - ${effectiveCustomer}` : 'POS বিক্রয়'
        })
        inv = inv.map(i => i.id === item.product.id
      ...
      setShowOk(true)
      setCart([]); setSelBuyer(null); setBuyerSearch(''); setManualCustomer(''); setCash(''); setShowCart(false)
  ...
  return (
    <div style={{
      ...
      {/* ── Search Row ── */}
      <div style={{ flexShrink: 0, background: 'var(--bg)', padding: '8px 16px 6px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface2)', border: '1.5px solid var(--border)', borderRadius: 12, padding: '0 12px', height: 42 }}>
          ...
          <button onClick={() => setShowCam(true)} style={{ flexShrink: 0, width: 42, height: 42, borderRadius: 12, background: 'linear-gradient(135deg,var(--primary),var(--accent))', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', cursor: 'pointer', boxShadow: '0 3px 10px rgba(255,87,34,0.4)' }}>
            <Camera size={18} />
          </button>
          <button onClick={() => setShowCart(true)} style={{ flexShrink: 0, position: 'relative', width: 42, height: 42, borderRadius: 12, background: count > 0 ? 'linear-gradient(135deg,#1a73e8,#0d47a1)' : 'var(--surface3)', border: count > 0 ? 'none' : '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: count > 0 ? 'white' : 'var(--text3)', cursor: 'pointer' }}>
            <ShoppingCart size={18} />
            {count > 0 && (
              <span style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, background: 'var(--danger)', borderRadius: '50%', color: 'white', fontSize: '0.6rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--bg)' }}>{count}</span>
            )}
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'scroll', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', padding: '6px 16px 110px' }}>
        {filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-title">পণ্য পাওয়া যায়নি</div>
          </div>
        ) : (
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.68rem', color: 'var(--text3)', fontFamily: 'var(--font-bn)', marginTop: 2 }}>পরিমাণ ও মূল্য পরিবর্তন করতে পারেন</div>
                </div>
            ...
            <div className="modal-body">
              {cart.length === 0 ? (
                ...
                <>
                  ...
                  {/* ✅ FIX 2: Due mode — buyer searchable dropdown with full list */}
                  {payMode === 'due' && (
                    <div className="input-group">
                      <label className="input-label"><User size={13} style={{ verticalAlign: 'middle', marginRight: 4 }} />ক্রেতা নির্বাচন *</label>
                      <div style={{ position: 'relative' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'var(--surface2)', border: `1.5px solid ${selBuyer ? 'var(--success)' : 'var(--border)'}`, borderRadius: 10, padding: '0 12px', height: 42, transition: 'border-color 0.2s' }}>
                          <User size={14} color="var(--text3)" style={{ flexShrink: 0 }} />
                          <input
                            value={buyerSearch}
                            onChange={e => { setBuyerSearch(e.target.value); setSelBuyer(null); setShowBuyerDD(true) }}
                            onFocus={() => setShowBuyerDD(true)}
                            placeholder="ক্রেতার নাম বা ফোন নম্বর..."
                            style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: 'var(--text)', fontSize: '0.83rem', fontFamily: 'var(--font-bn)' }}
                          />
                            ...
                            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, boxShadow: 'var(--shadow-lg)', zIndex: 20, maxHeight: 180, overflowY: 'auto', marginTop: 4 }}>
                              {fBuyers.length === 0 ? (
                                <div style={{ padding: '12px 14px', fontSize: '0.78rem', color: 'var(--text3)', fontFamily: 'var(--font-bn)', textAlign: 'center' }}>
                                  {buyers.length === 0 ? 'কোনো ক্রেতা নেই — Due Ledger এ যোগ করুন' : 'ক্রেতা পাওয়া যায়নি'}
                                </div>
                              ) : fBuyers.map(b => (
                                <button key={b.id}
                                  onClick={() => { setSelBuyer(b); setBuyerSearch(b.name); setShowBuyerDD(false) }}
                                  ...
                                  <span style={{ fontWeight: selBuyer?.id === b.id ? 700 : 400 }}>{b.name}</span>
                      ...
                      {selBuyer && (
                        <div style={{ fontSize: '0.72rem', color: 'var(--success)', marginTop: 5, fontFamily: 'var(--font-bn)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {/* Cash mode — optional customer name */}
                  {payMode === 'cash' && (
                    <>
                      <div className="input-group">
                        <label className="input-label"><User size={13} style={{ verticalAlign: 'middle', marginRight: 4 }} />ক্রেতার নাম (ঐচ্ছিক)</label>
                        <input className="input" placeholder="ক্রেতার নাম..." value={manualCustomer} onChange={e => setManualCustomer(e.target.value)} />
                      </div>
                      <div className="input-group">
            ...
            {cart.length > 0 && (
              <div className="modal-footer">
                <button className="btn btn-primary btn-full" onClick={checkout} disabled={processing} style={{ padding: 13, fontSize: '0.9rem' }}>
                  {processing
      ...
      {showOk && lastSale && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 24, padding: 28, width: '100%', maxWidth: 360, textAlign: 'center', animation: 'scale-in 0.35s cubic-bezier(0.34,1.56,0.64,1)' }}>
            ...
            <button className="btn btn-primary btn-full" onClick={() => { setShowOk(false); setLastSale(null) }} style={{ padding: 13 }}>
