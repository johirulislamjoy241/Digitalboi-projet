...
import { Search, Plus, Edit2, Trash2, ChevronLeft, ChevronRight, QrCode, RefreshCw, X, Package } from 'lucide-react'
...
function StatusBadge({ s }:{ s:string }) {
  ...
  return <span className="badge badge-neutral">📦 নিষ্ক্রিয়</span>
}
...
export default function InventorySection() {
  ...
  async function del(item:InventoryItem) {
    if (!confirm(`"${item.name}" স্থায়ীভাবে মুছে ফেলবেন?`)) return
    try { await api.deleteProduct(item.id); setInventory(inventory.filter(i=>i.id!==item.id)); toast('মুছে ফেলা হয়েছে','wa') }
    catch(e:unknown){toast((e as Error).message||'সমস্যা','er')}
  }

  // ✅ FIX 1: Archive option সম্পূর্ণ সরানো হয়েছে
  // শুধু status filter থেকে Archived বাদ দেওয়া হয়েছে
  const SF=['','In Stock','Low Stock','Out of Stock']
  const SL:Record<string,string>={'':'সব','In Stock':'✓ স্টক','Low Stock':'⚠ কম','Out of Stock':'✕ শেষ'}

  function doSort(k:SK) {
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div className="card card-p anim-fade-up">
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
          <div>
            <div className="section-title">📦 পণ্য তালিকা</div>
            <div className="section-subtitle">{inventory.length} টি পণ্য</div>
          </div>
        ...
        <div style={{display:'flex',gap:6,marginTop:8,overflowX:'auto',scrollbarWidth:'none'}}>
          {(['name','category','quantity','buy_price','sell_price'] as SK[]).map(k=>(
            <button key={k} className={`chip ${sortKey===k?'active':''}`} style={{flexShrink:0,fontSize:'0.62rem'}} onClick={()=>doSort(k)}>
              {k==='name'?'নাম':k==='category'?'ক্যাটাগরি':k==='quantity'?'পরিমাণ':k==='buy_price'?'ক্রয়':'বিক্রয়'}
      ...
      {!loading&&paginated.map((item,idx)=>(
        <div key={item.id} className="product-card anim-fade-up" style={{animationDelay:`${idx*0.03}s`}}>
          ...
          <div className="product-meta">
            ...
            {/* ✅ FIX 1: Archive বাটন সরানো হয়েছে — শুধু Edit, QR, Delete আছে */}
