...
export default function ReportsSection() {
  ...
  function exportCSV(type: 'inventory' | 'txns') {
    ...
    if (type === 'inventory') {
      rows = [
        ...
        // Fix #10: item.profit null-safe with fallback calculation
        ...inventory.map(i => [
          i.name, i.category, i.quantity, i.unit,
          i.buy_price, i.sell_price,
          i.quantity * i.buy_price,
          // Fix #10: use stored profit or calculate fallback
          i.profit ?? (i.sell_price - i.buy_price),
          i.status
        ])
      ]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div className="card card-p anim-fade-up">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-ghost btn-sm" onClick={() => exportCSV('inventory')}><Download size={14} /> পণ্য</button>
            <button className="btn btn-ghost btn-sm" onClick={() => exportCSV('txns')}><Download size={14} /> লেনদেন</button>
          </div>
        </div>
      </div>
      ...
      {/* Fix #10: profit null-safe with fallback */}
      <div className="card anim-fade-up anim-d5">
        ...
        {inventory.filter(i => i.status !== 'Archived')
          ...
          .map(item => (
            <div key={item.id} className="list-item">
              ...
              <div className="list-right">
                ...
                {/* Fix #10: null-safe profit — fallback to sell-buy */}
                <span className="text-xs text-muted mono">
                  {fmt((item.profit ?? (item.sell_price - item.buy_price)))}/একক
                </span>
              </div>
            </div>
          ))}
