...
function userThreshold(req: NextRequest) { return parseInt(req.headers.get('x-low-stock-threshold') || '10') || 10 }
...
export async function POST(req: NextRequest) {
  const uid = userId(req)
  if (!uid) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const threshold = userThreshold(req)
  ...
  // Fix #11: track inventory changes to return updated items to client
  const inventoryUpdates: { id: string; new_qty: number; status: string }[] = []
  ...
  if (product_id && quantity) {
    ...
    if (prod) {
      ...
      // Fix #9: use dynamic threshold here too
      ...
      // Fix #11: record the update so client can sync
      inventoryUpdates.push({ id: product_id, new_qty, status: newStatus })
      // Log transaction
  ...
  // Fix #11: return inventoryUpdates so client can sync state
  return NextResponse.json({ data, inventoryUpdates })
}
