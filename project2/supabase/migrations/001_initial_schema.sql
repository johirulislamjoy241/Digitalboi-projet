...
-- ═══════════════════════════════════════════════════════
--  Migration 002 — Security & Integrity Fixes
-- ═══════════════════════════════════════════════════════

-- Fix #20: Add buyer_id foreign key constraint to due_ledger
-- (run only if not already added)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'due_ledger_buyer_id_fkey'
    AND table_name = 'due_ledger'
  ) THEN
    ALTER TABLE due_ledger
      ADD CONSTRAINT due_ledger_buyer_id_fkey
      FOREIGN KEY (buyer_id) REFERENCES buyers(id) ON DELETE SET NULL;
  END IF;
END $$;

-- Fix #22: RLS Policies — service_role bypasses RLS, these protect against anon key misuse

-- Users table: only the user themselves can read their own row
...
-- Inventory: user can only CRUD their own rows
CREATE POLICY IF NOT EXISTS "inventory_owner" ON inventory
  FOR ALL USING (user_id::text = current_setting('request.jwt.claims', true)::json->>'sub');

-- Transactions: user can only read/write their own
CREATE POLICY IF NOT EXISTS "transactions_owner" ON transactions
  FOR ALL USING (user_id::text = current_setting('request.jwt.claims', true)::json->>'sub');

-- Due ledger: user can only manage their own entries
CREATE POLICY IF NOT EXISTS "due_ledger_owner" ON due_ledger
  FOR ALL USING (user_id::text = current_setting('request.jwt.claims', true)::json->>'sub');

-- Buyers: user can only manage their own buyers
CREATE POLICY IF NOT EXISTS "buyers_owner" ON buyers
  FOR ALL USING (user_id::text = current_setting('request.jwt.claims', true)::json->>'sub');

-- Registrations: user can only read their own profile
CREATE POLICY IF NOT EXISTS "registrations_owner" ON registrations
  FOR ALL USING (user_id::text = current_setting('request.jwt.claims', true)::json->>'sub');
