...
export default function AuthScreen(){
  ...
  async function doLogin(e:React.FormEvent){
    ...
    if(!lPh)return setLErr('Phone number is required.')
    if(!lPw)return setLErr('Password is required.')
  ...
  function nxt(){
    setRErr('')
    if(step===0){
      if(!reg.shopName)return setRErr('Business name is required.')
      if(!reg.shopType)return setRErr('Select business type.')
      if(!reg.country)return setRErr('Select country.')
      if(!reg.city)return setRErr('City is required.')
      if(!reg.address)return setRErr('Address is required.')
      if(!reg.shopPhone)return setRErr('Business phone is required.')
    }
    if(step===1){
      if(!reg.ownerName)return setRErr('Owner name is required.')
      if(!reg.ownerPhone)return setRErr('Personal phone is required.')
      if(!reg.nid)return setRErr('National ID is required.')
      if(!reg.dob)return setRErr('Date of birth is required.')
    }
    setStep(s=>s+1)
  }
  async function doReg(e:React.FormEvent){
    e.preventDefault();setRErr('')
    if(!rPh)return setRErr('Login phone is required.')
    if(!reg.password||reg.password.length<8)return setRErr('Password must be at least 8 characters.')
    if(reg.password!==cPw)return setRErr('Passwords do not match.')
    if(!ok)return setRErr('Please agree to the Terms of Service.')
  ...
  return(
    <div className="auth-sc">
      ...
      <div className="auth-w">
        ...
        {/* Card — content first, tabs at bottom */}
        <div className="auth-card">
          ...
          {tab==='login'&&(
            <form onSubmit={doLogin} style={{display:'flex',flexDirection:'column',gap:14}}>
              ...
              <button className="btn btn-p btn-full" type="submit" disabled={lLd} style={{padding:'12px',fontSize:'.87rem'}}>
          ...
          {tab==='register'&&(
            <div>
              ...
              <div style={{fontSize:'.68rem',color:'var(--t3)',textAlign:'center',marginBottom:14,fontWeight:700}}>
          ...
          {/* Divider */}
          <div style={{height:1,background:'var(--line2)',margin:'18px 0 16px'}}/>

          {/* TABS — at the bottom, like the screenshot */}
          <div className="auth-tabs">
            <button
              className={'auth-tab '+(tab==='login'?'on':'')}
              onClick={()=>{setTab('login');setStep(0);setLErr('')}}
            >
              <i className="fas fa-sign-in-alt"/> {t(lang,'signIn')}
            </button>
            <button
              className={'auth-tab '+(tab==='register'?'on':'')}
              onClick={()=>{setTab('register');setStep(0);setRErr('')}}
            >
        ...
        <div style={{textAlign:'center',marginTop:14,fontSize:'.65rem',color:'var(--t3)',fontWeight:700,letterSpacing:'.03em'}}>
          DigitalBoi POS v2.0 · Secure Cloud Business Management
        </div>
      </div>
    </div>
  )
