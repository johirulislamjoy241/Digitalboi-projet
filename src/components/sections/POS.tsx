    ...
    try {
      ...
      for (const item of cart) {
        const res = await api.doTransaction({
          ...
          notes: customer ? `POS - ${customer}` : 'POS বিক্রয়'
        })
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            ...
            style={{ flexShrink: 0, position: 'relative', width: 42, height: 42, borderRadius: 12, background: count > 0 ? 'linear-gradient(135deg,#1a73e8,#0d47a1)' : 'var(--surface3)', border: count > 0 ? 'none' : '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: count > 0 ? 'white' : 'var(--text3)', cursor: 'pointer' }}
          >
