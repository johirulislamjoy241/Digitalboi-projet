...
function CameraScanner({ onScan, onClose }: { onScan: (code: string) => void; onClose: () => void }) {
  ...
  const startCam = useCallback(async (fm: 'environment' | 'user') => {
    stop()
      ...
      videoRef.current.onloadedmetadata = async () => {
        ...
        if ('BarcodeDetector' in window) {
          try {
            ...
            const BC = (window as unknown as { BarcodeDetector: BC }).BarcodeDetector
            detectorRef.current = new BC({ formats: ['qr_code', 'code_128', 'ean_13', 'ean_8', 'code_39', 'code_93', 'upc_a', 'upc_e', 'itf', 'codabar', 'data_matrix', 'aztec'] })
            startLoop()
  ...
  useEffect(() => { startCam('environment'); return () => stop() }, []) // eslint-disable-line
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 900, background: '#000', display: 'flex', flexDirection: 'column' }}>
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -60%)', width: 260, height: 260 }}>
          ...
          <div style={{ position: 'absolute', left: 6, right: 6, top: `${scanY}%`, height: 2, background: 'linear-gradient(90deg,transparent,#FF5722 30%,#FF9800 50%,#FF5722 70%,transparent)', boxShadow: '0 0 10px rgba(255,87,34,0.9)', pointerEvents: 'none' }} />
        </div>
        {/* Toolbar */}
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '14px', paddingTop: 'max(14px,env(safe-area-inset-top))', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <button onClick={onClose} style={{ width: 42, height: 42, borderRadius: 13, background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><X size={20} /></button>
          ...
          <button onClick={() => { const nf = facing === 'environment' ? 'user' : 'environment'; setFacing(nf as 'environment'|'user'); startCam(nf as 'environment'|'user') }} style={{ width: 42, height: 42, borderRadius: 13, background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.2)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><FlipHorizontal size={18} /></button>
        </div>
        <div style={{ position: 'absolute', left: 0, right: 0, top: 'calc(50% + 145px)', textAlign: 'center' }}>
          <span style={{ background: 'rgba(0,0,0,0.65)', borderRadius: 20, padding: '6px 16px', color: 'rgba(255,255,255,0.9)', fontFamily: 'var(--font-bn)', fontSize: '0.78rem', display: 'inline-block' }}>
            {!camReady ? '📷 চালু হচ্ছে...' : noDetector ? '⌨️ নিচে লিখুন' : 'বারকোড ফ্রেমে ধরুন'}
          </span>
        </div>
      </div>
      {/* Bottom */}
      <div style={{ background: 'rgba(13,13,22,0.98)', padding: '14px 16px', paddingBottom: 'max(20px,env(safe-area-inset-bottom))', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        {noDetector && <div style={{ fontSize: '0.68rem', color: 'rgba(255,165,0,0.85)', fontFamily: 'var(--font-bn)', marginBottom: 8, textAlign: 'center' }}>⚠ এই ব্রাউজারে স্বয়ংক্রিয় স্ক্যান নেই</div>}
        <div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.4)', fontFamily: 'var(--font-bn)', marginBottom: 8, textAlign: 'center' }}>বা ম্যানুয়ালি বারকোড / পণ্যের নাম লিখুন</div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <input
            ...
            autoComplete="off"
          />
          <button onClick={() => manualCode.trim() && onScan(manualCode.trim())} style={{ padding: '0 18px', background: 'var(--primary)', border: 'none', borderRadius: 12, color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Check size={20} /></button>
        </div>
        <button onClick={onClose} style={{ width: '100%', padding: '11px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, color: 'rgba(255,255,255,0.65)', cursor: 'pointer', fontFamily: 'var(--font-bn)', fontSize: '0.82rem', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><X size={15} /> বাতিল</button>
      </div>
    </div>
  )
}
...
export default function POSSection() {
  ...
  /* ── Issue 7 fix: scan → directly add to cart, no popup ── */
  function handleBarcodeScan(code: string) {
    setShowCamera(false)
    const q = code.trim()
    if (!q) return
    const ql = q.toLowerCase()
    // Match: exact product_link, then partial product_link, then id, then name
    const found =
      inventory.find(i => i.status !== 'Archived' && i.product_link && i.product_link.trim() === q) ||
      inventory.find(i => i.status !== 'Archived' && i.product_link && i.product_link.toLowerCase().includes(ql)) ||
      inventory.find(i => i.status !== 'Archived' && i.id === q) ||
      inventory.find(i => i.status !== 'Archived' && i.name.toLowerCase() === ql) ||
      inventory.find(i => i.status !== 'Archived' && i.name.toLowerCase().includes(ql))

    if (found) {
      addToCartDirect(found)
      toast(`✅ ${found.name} — কার্টে যোগ হয়েছে`, 'ok')
    } else {
      toast(`❌ পণ্য পাওয়া যায়নি: "${q}"`, 'er')
    }
  }
    ...
    try {
      ...
      for (const item of cart) {
        ...
        updInv = updInv.map(i => i.id === item.product.id
          ? { ...i, quantity: res.new_qty, status: (res.new_qty <= 0 ? 'Out of Stock' : res.new_qty <= 10 ? 'Low Stock' : 'In Stock') as InventoryItem['status'] } : i)
      ...
      setShowSuccess(true)
      setCart([]); setCustomerName(''); setCashGiven(''); setShowCart(false)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - var(--topbar-h) - var(--nav-h) - env(safe-area-inset-bottom))', overflow: 'hidden', margin: '-16px' }}>
      {showCamera && <CameraScanner onScan={handleBarcodeScan} onClose={() => setShowCamera(false)} />}

      {/* ── Issue 2 fix: search bar row — proper flex, no overflow ── */}
      <div style={{ padding: '10px 10px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {/* Search — takes remaining space */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface2)', border: '1.5px solid var(--border)', borderRadius: 12, padding: '9px 12px', flex: 1, minWidth: 0, transition: 'all 0.2s' }}>
            ...
            />
            {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', padding: 0, flexShrink: 0, lineHeight: 1 }}>✕</button>}
          </div>
          {/* Camera scan button */}
          <button
            onClick={() => setShowCamera(true)}
            title="ক্যামেরা স্ক্যান"
            style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg, var(--primary), var(--accent))', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', cursor: 'pointer', boxShadow: '0 4px 12px rgba(255,87,34,0.35)' }}
          ><Camera size={17} /></button>
          {/* Cart button */}
          <button
            onClick={() => setShowCart(true)}
            style={{ position: 'relative', width: 42, height: 42, borderRadius: 12, flexShrink: 0, background: cartCount > 0 ? 'linear-gradient(135deg,#1a73e8,#0d47a1)' : 'var(--surface3)', border: cartCount > 0 ? 'none' : '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: cartCount > 0 ? 'white' : 'var(--text3)', cursor: 'pointer', boxShadow: cartCount > 0 ? '0 4px 12px rgba(26,115,232,0.35)' : 'none' }}
          >
            <ShoppingCart size={17} />
            {cartCount > 0 && <span style={{ position: 'absolute', top: -5, right: -5, width: 18, height: 18, background: 'var(--danger)', borderRadius: '50%', color: 'white', fontSize: '0.58rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--bg)' }}>{cartCount}</span>}
          </button>
        </div>
      </div>

      {/* Product grid */}
      <div style={{ flex: 1, overflowY: 'scroll', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', touchAction: 'pan-y', padding: '10px 10px 0', paddingBottom: 100 }}>
        {filtered.length === 0
              ...
              {filtered.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '1.3rem', lineHeight: 1 }}>{CAT[item.category] || '📦'}</div>
      ...
      {cartCount > 0 && !showCart && (
        <div onClick={() => setShowCart(true)} style={{ position: 'fixed', bottom: 'calc(var(--nav-h) + env(safe-area-inset-bottom) + 12px)', left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 32px)', maxWidth: 380, background: 'linear-gradient(135deg,var(--primary),var(--accent))', borderRadius: 16, padding: '12px 16px', boxShadow: '0 8px 24px rgba(255,87,34,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 140, cursor: 'pointer' }}>
          <div style={{ color: 'white' }}>
            <div style={{ fontSize: '0.68rem', opacity: 0.85, fontFamily: 'var(--font-bn)' }}>কার্টে {cartCount} পণ্য</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: '1.05rem' }}>{fmt(cartTotal)}</div>
          </div>
            ...
            {cart.length === 0
              ...
              : <>
                  {cart.map(item => (
                    <div key={item.product.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'var(--surface2)', borderRadius: 10, padding: '4px 6px', border: '1px solid var(--border)', flexShrink: 0 }}>
                        ...
                        <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '0.82rem', flexShrink: 0, minWidth: 58, textAlign: 'right' }}>{fmt(item.qty * item.unitPrice)}</span>
                      </div>
                    </div>
                  ))}
                  <div style={{ background: 'var(--surface2)', borderRadius: 12, padding: '12px 14px', marginTop: 12, marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                      <span style={{ fontSize: '0.78rem', color: 'var(--text2)', fontFamily: 'var(--font-bn)' }}>সর্বমোট</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: '1.05rem' }}>{fmt(cartTotal)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: '0.7rem', color: 'var(--text3)', fontFamily: 'var(--font-bn)' }}>লাভ</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', fontWeight: 600, color: cartProfit >= 0 ? 'var(--success)' : 'var(--danger)' }}>{cartProfit >= 0 ? '+' : ''}{fmt(cartProfit)}</span>
                    </div>
                  </div>
                  <div className="input-group">
                    <label className="input-label"><User size={12} style={{ verticalAlign: 'middle', marginRight: 4 }} />ক্রেতার নাম {payMode === 'due' ? '*' : '(ঐচ্ছিক)'}</label>
                    <input className="input" placeholder="ক্রেতার নাম..." value={customerName} onChange={e => setCustomerName(e.target.value)} />
                  </div>
                  <div className="tabs" style={{ marginBottom: 10 }}>
                    <button className={`tab ${payMode === 'cash' ? 'active' : ''}`} onClick={() => setPayMode('cash')}><Banknote size={12} style={{ verticalAlign: 'middle', marginRight: 3 }} />নগদ</button>
                    <button className={`tab ${payMode === 'due' ? 'active' : ''}`} onClick={() => setPayMode('due')}><CreditCard size={12} style={{ verticalAlign: 'middle', marginRight: 3 }} />বকেয়া</button>
                  </div>
                  {payMode === 'cash' && (
                    <div className="input-group">
                  ...
                  <button className="btn btn-primary btn-full" onClick={checkout} disabled={processing} style={{ padding: 14, fontSize: '0.9rem' }}>
                    {processing ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><span className="spin" style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: 'white', borderRadius: '50%', display: 'inline-block' }} />প্রসেস...</span>
