...
import { Search, Plus, BookOpen, User, Package, CreditCard, ChevronDown, Trash2, RefreshCw } from 'lucide-react'
...
export default function DueLedgerSection() {
  ...
  // total_amount from qty * unitPrice
  ...
  async function addDue() {
    ...
    try {
      const payload = {
        ...
        product_name: selProduct?.name || 'বিবিধ',
        quantity: parseFloat(dueForm.qty),
        unit: dueForm.unit,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div className="card card-p anim-fade-up anim-d1">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={() => setModal('add')} title="নতুন বকেয়া"><Plus size={16} /></button>
          <button className="btn btn-ghost btn-sm" onClick={() => setModal('buyer')} title="নতুন ক্রেতা"><User size={16} /></button>
          <button className="btn btn-ghost btn-sm" onClick={load} title="রিফ্রেশ"><RefreshCw size={16} className={loading ? 'spin' : ''} /></button>
        </div>
      </div>
              ...
              <div key={entry.id} className={`due-card ${cardClass} anim-fade-up`}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div style={{ flex: 1, minWidth: 0, marginRight: 10 }}>
                    ...
                    <div style={{ fontSize: '0.72rem', color: 'var(--text3)', fontFamily: 'var(--font-bn)' }}>
                      {entry.product_name} · {entry.quantity} {entry.unit}
                    </div>
                    {entry.due_date && (
                      <div style={{ fontSize: '0.68rem', color: isOverdue ? 'var(--danger)' : 'var(--text3)', marginTop: 2, fontFamily: 'var(--font-bn)' }}>
                        {isOverdue ? '⚠ মেয়াদ শেষ: ' : '📅 '}{formatDateShort(entry.due_date)}
                      </div>
                    )}
                    {entry.notes && <div style={{ fontSize: '0.68rem', color: 'var(--text3)', marginTop: 2, fontFamily: 'var(--font-bn)' }}>💬 {entry.notes}</div>}
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: '0.95rem', color: entry.status === 'Paid' ? 'var(--success)' : 'var(--danger)' }}>
                      {fmt(entry.remaining)}
                    </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  ...
                  <button className="btn btn-danger btn-xs" onClick={() => deleteDue(entry)} style={{ width: 34, padding: 0 }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
      ...
      {modal === 'add' && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxHeight: '92dvh', overflowY: 'auto' }}>
            ...
            <div className="modal-title">📝 নতুন বকেয়া যোগ</div>
            ...
            <div className="input-group">
              ...
              <div style={{ position: 'relative' }}>
                <div className="search-bar" style={{ marginBottom: 0 }}>
                  ...
                  <input
                    value={prodSearch}
                    onChange={e => { setProdSearch(e.target.value); setSelProduct(null); setShowProdDd(true) }}
            ...
            {dueForm.qty && dueForm.unitPrice && calcTotal > 0 && (
              <div style={{ background: 'var(--primary-bg)', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: '0.78rem', fontFamily: 'var(--font-bn)', color: 'var(--text2)' }}>মোট মূল্য</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--text)' }}>{fmt(calcTotal)}</span>
                </div>
      ...
      {modal === 'payment' && editEntry && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-handle" />
            <div className="modal-title">💳 পেমেন্ট গ্রহণ</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button className="btn btn-primary" style={{ flex: 2 }} onClick={doPayment} disabled={saving}>
                {saving ? 'সংরক্ষণ হচ্ছে...' : '✅ পেমেন্ট নিশ্চিত করুন'}
              </button>
            </div>
          </div>
        </div>
      ...
      {modal === 'buyer' && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-handle" />
            <div className="modal-title">👤 নতুন ক্রেতা যোগ</div>
            <div className="input-group">
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn btn-ghost" style={{ flex: 1 }} onClick={() => setModal(null)}>বাতিল</button>
              <button className="btn btn-primary" style={{ flex: 2 }} onClick={addBuyer} disabled={saving}>{saving ? 'সংরক্ষণ হচ্ছে...' : 'ক্রেতা যোগ করুন'}</button>
            </div>
          </div>
        </div>
      )}
