...
type DT = 'all'|'pending'|'partial'|'overdue'|'paid'|'buyers'

/* ══ BUYER PROFILE SHEET ══ */
function BuyerProfile({ buyer, dues, fmt, onClose }:{ buyer:Buyer; dues:DueLedgerEntry[]; fmt:(v:number)=>string; onClose:()=>void }) {
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div style={{background:'var(--surface2)',borderRadius:12,padding:12,marginBottom:12,display:'flex',flexDirection:'column',gap:7}}>
            {buyer.phone&&<div style={{display:'flex',alignItems:'center',gap:8}}><Phone size={13} color="var(--info)"/><span style={{fontSize:'0.82rem',color:'var(--text)',fontFamily:'var(--font-bn)'}}>{buyer.phone}</span></div>}
            {buyer.address&&<div style={{display:'flex',alignItems:'flex-start',gap:8}}><MapPin size={13} color="var(--primary)" style={{flexShrink:0,marginTop:2}}/><span style={{fontSize:'0.82rem',color:'var(--text)',fontFamily:'var(--font-bn)'}}>{buyer.address}</span></div>}
            {buyer.notes&&<div style={{display:'flex',alignItems:'flex-start',gap:8}}><FileText size={13} color="var(--text3)" style={{flexShrink:0,marginTop:2}}/><span style={{fontSize:'0.78rem',color:'var(--text2)',fontFamily:'var(--font-bn)'}}>{buyer.notes}</span></div>}
            {!buyer.phone&&!buyer.address&&!buyer.notes&&<div style={{fontSize:'0.75rem',color:'var(--text3)',fontFamily:'var(--font-bn)',textAlign:'center'}}>কোনো অতিরিক্ত তথ্য নেই</div>}
          </div>
          {/* Stats */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
            {[
              {label:`মোট বকেয়া (${bd.filter(e=>e.status!=='Paid').length}টি)`,val:fmt(totalDue),color:'var(--danger)',bg:'var(--danger-light)'},
              {label:'মোট পরিশোধ',val:fmt(totalPaid),color:'var(--success)',bg:'var(--success-light)'},
              {label:'মোট লেনদেন',val:fmt(totalAmt),color:'var(--primary)',bg:'var(--primary-bg)'},
              {label:'মোট রেকর্ড',val:String(bd.length),color:'var(--text)',bg:'var(--surface2)'},
            ].map((s,i)=>(
              <div key={i} style={{background:s.bg,borderRadius:12,padding:'10px 12px',textAlign:'center'}}>
                <div style={{fontFamily:'var(--font-mono)',fontWeight:800,fontSize:'0.95rem',color:s.color}}>{s.val}</div>
          ...
          {bd.length===0?(
            <div style={{textAlign:'center',padding:'12px 0',fontSize:'0.78rem',color:'var(--text3)',fontFamily:'var(--font-bn)'}}>কোনো লেনদেন নেই</div>
          ):(
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}>
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:'flex',alignItems:'center',gap:4,marginTop:2}}>
                          ...
                          <span style={{fontSize:'0.62rem',color:'var(--text3)',fontFamily:'var(--font-bn)'}}>{formatDateShort(e.created_at)}{e.due_date&&` · মেয়াদ: ${formatDateShort(e.due_date)}`}</span>
                        </div>
                      </div>
                      <div style={{textAlign:'right',flexShrink:0}}>
                        ...
                        <span className={`badge ${e.status==='Paid'?'badge-success':e.status==='Partial'?'badge-warning':'badge-danger'}`} style={{fontSize:'0.58rem'}}>{e.status==='Paid'?'পরিশোধ':e.status==='Partial'?'আংশিক':'বকেয়া'}</span>
                      </div>
                    </div>
...
export default function DueLedgerSection() {
  ...
  const [tab, setTab] = useState<DT>('all')
  ...
  const [dueForm, setDueForm] = useState({ qty:'', unit:'pcs', price:'', paid:'0', date:'', notes:'' })
  ...
  async function load() {
    ...
    setDueLedger(entries); setBuyers(bl)
  ...
  const summary = useMemo(()=>{
    ...
    dueLedger.forEach(e=>{ if(e.status!=='Paid'){ total+=e.remaining; if(e.due_date&&new Date(e.due_date)<today) overdue+=e.remaining } })
  ...
  const fBuyers = useMemo(()=>{ if(!buyerSearch) return buyers.slice(0,20); return buyers.filter(b=>b.name.toLowerCase().includes(buyerSearch.toLowerCase())||(b.phone||'').includes(buyerSearch)) },[buyers,buyerSearch])
  const fProds = useMemo(()=>{ if(!prodSearch) return inventory.filter(i=>i.status!=='Archived').slice(0,20); return inventory.filter(i=>i.status!=='Archived'&&i.name.toLowerCase().includes(prodSearch.toLowerCase())) },[inventory,prodSearch])

  const calcTotal=(parseFloat(dueForm.qty)||0)*(parseFloat(dueForm.price)||0)
  const calcRem=calcTotal-(parseFloat(dueForm.paid)||0)

  async function addDue() {
    if(!selBuyer) return toast('ক্রেতা নির্বাচন করুন','er')
    if(!dueForm.price) return toast('একক মূল্য দিন','er')
    if(!dueForm.qty) return toast('পরিমাণ দিন','er')
    setSaving(true)
    try {
      const created=await api.addDueEntry({ buyer_id:selBuyer.id, buyer_name:selBuyer.name, product_id:selProd?.id, product_name:selProd?.name||'বিবিধ', quantity:parseFloat(dueForm.qty), unit:dueForm.unit, unit_price:parseFloat(dueForm.price), total_amount:calcTotal, paid_amount:parseFloat(dueForm.paid)||0, due_date:dueForm.date||undefined, notes:dueForm.notes })
      setDueLedger([created,...dueLedger])
      toast('বকেয়া যোগ হয়েছে ✅','ok')
      setModal(null); setSelBuyer(null); setBuyerSearch(''); setSelProd(null); setProdSearch(''); setDueForm({qty:'',unit:'pcs',price:'',paid:'0',date:'',notes:''})
  ...
  async function addBuyer() {
    ...
    try {
      ...
      setBuyers([created,...buyers]); toast('ক্রেতা যোগ হয়েছে ✅','ok'); setModal(null); setBuyerForm({name:'',phone:'',email:'',address:'',notes:''})
    } catch(e:unknown){toast((e as Error).message||'সমস্যা','er')}
    setSaving(false)
  }

  async function delDue(e:DueLedgerEntry) {
    if(!confirm(`"${e.buyer_name}" এর বকেয়া মুছবেন?`)) return
    try { await api.deleteDueEntry(e.id); setDueLedger(dueLedger.filter(x=>x.id!==e.id)); toast('মুছে ফেলা হয়েছে','wa') }
    catch(e:unknown){toast((e as Error).message||'সমস্যা','er')}
  }

  const TABS:[DT,string][]=[['all','সব'],['pending','বকেয়া'],['partial','আংশিক'],['overdue','⚠ মেয়াদ'],['paid','✅ পরিশোধ'],['buyers','👥 ক্রেতা']]
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div className="card card-p anim-fade-up anim-d1">
        ...
        {/* Responsive row — search grows, icons fixed */}
        <div style={{display:'flex',alignItems:'center',gap:7,overflow:'hidden'}}>
          <div style={{flex:'1 1 0',display:'flex',alignItems:'center',gap:7,background:'var(--surface2)',border:'1.5px solid var(--border)',borderRadius:12,padding:'0 10px',height:42,minWidth:0,overflow:'hidden'}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="নাম বা পণ্য খুঁজুন..." style={{flex:1,background:'transparent',border:'none',outline:'none',color:'var(--text)',fontSize:'0.82rem',fontFamily:'var(--font-bn)',minWidth:0,width:'100%'}}/>
          ...
          <button className="btn btn-primary btn-icon" onClick={()=>setModal('add')} title="নতুন বকেয়া" style={{width:42,height:42,minWidth:42,flexShrink:0}}><Plus size={18}/></button>
          <button className="btn btn-ghost btn-icon" onClick={()=>setModal('buyer')} title="নতুন ক্রেতা" style={{width:42,height:42,minWidth:42,flexShrink:0}}><User size={16}/></button>
          <button className="btn btn-ghost btn-icon" onClick={load} style={{width:42,height:42,minWidth:42,flexShrink:0}}><RefreshCw size={15} className={loading?'spin':''}/></button>
        </div>
      </div>
              ...
              <div key={entry.id} className={`due-card ${cls} anim-fade-up`}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                  <div style={{flex:1,minWidth:0,marginRight:10}}>
                    ...
                    {entry.due_date&&<div style={{fontSize:'0.68rem',color:ov?'var(--danger)':'var(--text3)',marginTop:2,fontFamily:'var(--font-bn)'}}>{ov?'⚠ মেয়াদ শেষ: ':'📅 '}{formatDateShort(entry.due_date)}</div>}
                  ...
                  <div style={{textAlign:'right',flexShrink:0}}>
                    ...
                    <span className={`badge ${entry.status==='Paid'?'badge-success':entry.status==='Partial'?'badge-warning':'badge-danger'}`} style={{marginTop:4,display:'inline-block'}}>{entry.status==='Paid'?'পরিশোধ':entry.status==='Partial'?'আংশিক':'বকেয়া'}</span>
                  </div>
                </div>
                <div className="progress-bar">
                  <div className="progress-fill" style={{width:`${pct}%`,background:pct===100?'var(--success)':pct>50?'var(--warning)':'var(--danger)'}}/>
                ...
                <div style={{display:'flex',gap:8,marginTop:10}}>
                  {entry.status!=='Paid'&&<button className="btn btn-primary btn-sm" style={{flex:1}} onClick={()=>{setEditEntry(entry);setModal('payment')}}><CreditCard size={14}/> পেমেন্ট নিন</button>}
                  <button className="btn btn-danger btn-xs" onClick={()=>delDue(entry)} style={{width:34,padding:0}}><Trash2 size={14}/></button>
                </div>
              </div>
            )
          })}
        ...
        <div className="modal-overlay" onClick={()=>setModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
                  ...
                  {showPDD&&fProds.length>0&&(
                    <>
                      ...
                      <div style={{position:'absolute',top:'100%',left:0,right:0,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,boxShadow:'var(--shadow-lg)',zIndex:20,maxHeight:150,overflowY:'auto',marginTop:4}}>
                        {fProds.map(p=>(
                          <button key={p.id} onClick={()=>{setSelProd(p);setProdSearch(p.name);setDueForm(f=>({...f,price:String(p.sell_price),unit:p.unit}));setShowPDD(false)}} style={{width:'100%',padding:'10px 14px',textAlign:'left',background:'none',border:'none',cursor:'pointer',borderBottom:'1px solid var(--border)',fontSize:'0.82rem',fontFamily:'var(--font-bn)',color:'var(--text)'}}>
              ...
              <div className="grid-2">
                ...
                <div className="input-group">
                  ...
                  <input className="input" type="number" placeholder="0.00" value={dueForm.price} onChange={e=>setDueForm(f=>({...f,price:e.target.value}))} step="any"/>
                </div>
              </div>
              <div className="grid-2">
                <div className="input-group">
                  <label className="input-label">অগ্রিম পরিশোধ</label>
                  <input className="input" type="number" placeholder="0" value={dueForm.paid} onChange={e=>setDueForm(f=>({...f,paid:e.target.value}))} min="0"/>
                </div>
                <div className="input-group">
                  <label className="input-label">শেষ তারিখ</label>
                  <input className="input" type="date" value={dueForm.date} onChange={e=>setDueForm(f=>({...f,date:e.target.value}))}/>
                </div>
              </div>
              ...
              {dueForm.qty&&dueForm.price&&calcTotal>0&&(
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" style={{flex:2}} onClick={addDue} disabled={saving}>{saving?'সংরক্ষণ...':'✅ বকেয়া যোগ করুন'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal==='buyer'&&(
        <div className="modal-overlay" onClick={()=>setModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-footer">
              <button className="btn btn-ghost" style={{flex:1}} onClick={()=>setModal(null)}>বাতিল</button>
              <button className="btn btn-primary" style={{flex:2}} onClick={addBuyer} disabled={saving}>{saving?'সংরক্ষণ...':'✅ ক্রেতা যোগ করুন'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ══ BUYER PROFILE ══ */}
      {profileBuyer&&<BuyerProfile buyer={profileBuyer} dues={dueLedger} fmt={fmt} onClose={()=>setProfileBuyer(null)}/>}
