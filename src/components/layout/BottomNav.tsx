...
export default function BottomNav() {
  ...
  function nav(id: ActiveSection, isScan?: boolean) {
    if (isScan) {
      toast('QR স্ক্যানার শীঘ্রই আসছে 📷', 'in')
      return
    }
    setActiveSection(id)
  }
        ...
        if (item.isScan) {
          return (
            <div key={item.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, flex: 1 }}>
              <button className="nav-scan-btn" onClick={() => nav(item.id, true)} aria-label="QR Scan">
                <Icon size={22} strokeWidth={2.5} />
              </button>
              <span className="nav-item-label" style={{ color: 'var(--text3)' }}>{item.label}</span>
            </div>
          )
        }
        return (
