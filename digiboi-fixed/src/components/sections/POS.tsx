...
const CE: Record<string,string> = { General:'📦',Electronics:'⚡',Clothing:'👕','Food & Beverage':'🍎',Furniture:'🪑',Books:'📚',Stationery:'✏️',Medicine:'💊',Cosmetics:'💄',Hardware:'🔧',Toys:'🎮',Sports:'⚽',Automotive:'🚗',Agriculture:'🌾',Other:'📋' }

/* ══ CAMERA SCANNER ══ */
function CameraScanner({ onScan, onClose }:{ onScan:(c:string)=>void; onClose:()=>void }) {
  const vRef = useRef<HTMLVideoElement>(null)
  const sRef = useRef<MediaStream|null>(null)
  const aRef = useRef<number>(0)
  const dRef = useRef<unknown>(null)
  ...
  const stop = useCallback(()=>{ cancelAnimationFrame(aRef.current); sRef.current?.getTracks().forEach(t=>t.stop()); sRef.current=null },[])

  const detect = useCallback(async()=>{
    if(!vRef.current||!dRef.current||done.current) return
    type D={detect:(v:HTMLVideoElement)=>Promise<Array<{rawValue:string}>>}
    const det=dRef.current as D
    const loop=async()=>{
      if(done.current||!vRef.current) return
      try {
        if(vRef.current.readyState>=2){
          const r=await det.detect(vRef.current)
          if(r.length>0&&r[0].rawValue){ done.current=true; stop(); onScan(r[0].rawValue.trim()); return }
        }
      } catch{/**/}
  ...
  const start = useCallback(async(fm:'environment'|'user')=>{
    stop(); done.current=false; setReady(false)
    try {
      const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:fm,width:{ideal:1280},height:{ideal:720}}})
      sRef.current=stream
      if(!vRef.current) return
      vRef.current.srcObject=stream
      vRef.current.onloadedmetadata=async()=>{
        try{ await vRef.current?.play() } catch{ return }
        setReady(true)
        if('BarcodeDetector' in window){
          try{
            type BC=new(o:object)=>unknown
            const Cls=(window as unknown as{BarcodeDetector:BC}).BarcodeDetector
            dRef.current=new Cls({formats:['qr_code','code_128','ean_13','ean_8','code_39','upc_a','data_matrix']})
            detect()
          } catch{setNoApi(true)}
        } else setNoApi(true)
      }
    } catch{ setNoApi(true); setReady(true) }
  },[stop,detect])

  useEffect(()=>{ start('environment'); return ()=>stop() },[]) // eslint-disable-line

  const [manual, setManual] = useState('')
  function submit(){ const c=manual.trim(); if(c) onScan(c) }

  return (
    <div style={{position:'fixed',inset:0,zIndex:900,background:'#000',display:'flex',flexDirection:'column'}}>
      <div style={{flex:1,position:'relative',overflow:'hidden'}}>
        <video ref={vRef} playsInline muted autoPlay style={{width:'100%',height:'100%',objectFit:'cover'}}/>
        ...
        <div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-60%)',width:260,height:260}}>
          {[{top:-2,left:-2,borderTop:'3px solid #FF5722',borderLeft:'3px solid #FF5722',borderRadius:'6px 0 0 0'},{top:-2,right:-2,borderTop:'3px solid #FF5722',borderRight:'3px solid #FF5722',borderRadius:'0 6px 0 0'},{bottom:-2,left:-2,borderBottom:'3px solid #FF5722',borderLeft:'3px solid #FF5722',borderRadius:'0 0 0 6px'},{bottom:-2,right:-2,borderBottom:'3px solid #FF5722',borderRight:'3px solid #FF5722',borderRadius:'0 0 6px 0'}].map((s,i)=>(
            <div key={i} style={{position:'absolute',width:34,height:34,...s}}/>
          ))}
          <div style={{position:'absolute',left:6,right:6,top:`${scanY}%`,height:2,background:'linear-gradient(90deg,transparent,#FF5722 30%,#FF9800 50%,#FF5722 70%,transparent)',boxShadow:'0 0 10px rgba(255,87,34,0.9)',borderRadius:2,pointerEvents:'none'}}/>
        </div>
        {/* Top bar */}
        <div style={{position:'absolute',top:0,left:0,right:0,padding:'14px',paddingTop:'max(14px,env(safe-area-inset-top))',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <div style={{background:'rgba(0,0,0,0.6)',borderRadius:20,padding:'8px 16px',display:'flex',alignItems:'center',gap:8}}>
            ...
            <span style={{color:'white',fontFamily:'var(--font-bn)',fontSize:'0.85rem',fontWeight:600}}>স্ক্যান → সরাসরি কার্টে</span>
          </div>
          <button onClick={()=>{ const nf=facing==='environment'?'user':'environment'; setFacing(nf); start(nf) }} style={{width:42,height:42,borderRadius:13,background:'rgba(0,0,0,0.6)',border:'1px solid rgba(255,255,255,0.2)',color:'white',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>
        ...
        <div style={{position:'absolute',left:0,right:0,top:'calc(50% + 145px)',textAlign:'center'}}>
          <span style={{background:'rgba(0,0,0,0.65)',borderRadius:20,padding:'6px 16px',color:'rgba(255,255,255,0.9)',fontFamily:'var(--font-bn)',fontSize:'0.78rem',display:'inline-block'}}>
            {!ready?'📷 ক্যামেরা চালু...':noApi?'⌨️ নিচে ম্যানুয়াল লিখুন':'🔍 বারকোড ফ্রেমে ধরুন'}
          </span>
        </div>
      </div>
      {/* Bottom */}
      <div style={{background:'rgba(13,13,22,0.98)',padding:'14px 16px',paddingBottom:'max(20px,env(safe-area-inset-bottom))',borderTop:'1px solid rgba(255,255,255,0.08)'}}>
        {noApi&&<div style={{fontSize:'0.68rem',color:'rgba(255,165,0,0.85)',fontFamily:'var(--font-bn)',marginBottom:8,textAlign:'center'}}>⚠ এই ব্রাউজারে স্বয়ংক্রিয় স্ক্যান নেই</div>}
        <div style={{fontSize:'0.68rem',color:'rgba(255,255,255,0.4)',fontFamily:'var(--font-bn)',marginBottom:8,textAlign:'center'}}>ম্যানুয়াল কোড লিখুন</div>
        <div style={{display:'flex',gap:8,marginBottom:10}}>
          <input style={{flex:1,padding:'12px 14px',background:'rgba(255,255,255,0.1)',border:'1.5px solid rgba(255,255,255,0.15)',borderRadius:12,color:'white',fontSize:'0.88rem',fontFamily:'var(--font-bn)',outline:'none'}} placeholder="বারকোড / পণ্যের নাম..." value={manual} onChange={e=>setManual(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()} autoComplete="off"/>
          <button onClick={submit} style={{padding:'0 18px',background:'var(--primary)',border:'none',borderRadius:12,color:'white',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><Check size={20}/></button>
        </div>
...
export default function POSSection() {
  ...
  useEffect(()=>{ if(inventory.length===0) api.getInventory().then(setInventory) },[]) // eslint-disable-line

  /* Scan → directly add to cart (Issue #7) */
  function onScan(code:string) {
    setShowCam(false)
    const q=code.trim().toLowerCase()
    if(!q) return
    const found=inventory.find(i=>i.status!=='Archived'&&((i.product_link&&i.product_link.trim()===code.trim())||(i.product_link&&i.product_link.toLowerCase().includes(q))||i.id===code||i.name.toLowerCase()===q))
      ||inventory.find(i=>i.status!=='Archived'&&i.name.toLowerCase().includes(q))
    if(found){
      if(found.quantity<=0){ toast(`⚠ "${found.name}" স্টক শেষ`,'er'); return }
      addItem(found)
  ...
  const filtered = useMemo(()=>{
    ...
    return active.filter(i=>i.name.toLowerCase().includes(search.toLowerCase())||i.category.toLowerCase().includes(search.toLowerCase()))
  },[inventory,search])

  function addItem(p:InventoryItem) {
    if(p.quantity<=0){ toast('স্টক শেষ!','er'); return }
    setCart(prev=>{
      const ex=prev.find(c=>c.product.id===p.id)
      if(ex){
        if(ex.qty>=p.quantity){ toast('স্টক পর্যাপ্ত নেই','er'); return prev }
        return prev.map(c=>c.product.id===p.id?{...c,qty:c.qty+1}:c)
      }
      return [...prev,{product:p,qty:1,unitPrice:p.sell_price}]
    })
  }
  ...
  function price(id:string,v:string){ setCart(prev=>prev.map(c=>c.product.id===id?{...c,unitPrice:parseFloat(v)||0}:c)) }

  const total=cart.reduce((s,c)=>s+c.qty*c.unitPrice,0)
  const profit=cart.reduce((s,c)=>s+c.qty*(c.unitPrice-c.product.buy_price),0)
  ...
  async function checkout() {
    ...
    if(payMode==='due'&&!customer) return toast('বকেয়ার জন্য ক্রেতার নাম দিন','er')
    setProcessing(true)
    try {
      let inv=[...inventory]
      const txns:Transaction[]=[]
      for(const item of cart){
        const res=await api.doTransaction({product_id:item.product.id,product_name:item.product.name,txn_type:'out',quantity:item.qty,unit:item.product.unit,buy_price:item.product.buy_price,sell_price:item.unitPrice,notes:customer?`POS - ${customer}`:'POS বিক্রয়'})
        inv=inv.map(i=>i.id===item.product.id?{...i,quantity:res.new_qty,status:(res.new_qty<=0?'Out of Stock':res.new_qty<=10?'Low Stock':'In Stock') as InventoryItem['status']}:i)
        txns.push(res.data)
      }
      setInventory(inv)
      setTransactions([...txns,...transactions])
      setLastSale({total,profit,change:Math.max(0,change)})
      setShowOk(true)
      setCart([]); setCustomer(''); setCash(''); setShowCart(false)
      toast(`✅ বিক্রয় সম্পন্ন! মোট: ${fmt(total)}`,'ok')
    } catch(e:unknown){toast((e as Error).message||'সমস্যা','er')}
    setProcessing(false)
  }

  return (
    <div style={{display:'flex',flexDirection:'column',height:'calc(100dvh - var(--topbar-h) - var(--nav-h) - env(safe-area-inset-bottom))',overflow:'hidden',margin:'-16px',boxSizing:'border-box',width:'calc(100% + 32px)',maxWidth:'calc(430px + 32px)'}}>

      {showCam&&<CameraScanner onScan={onScan} onClose={()=>setShowCam(false)}/>}

      {/* ── Search Row ── */}
      <div style={{display:'flex',alignItems:'center',gap:6,padding:'8px 10px 6px',flexShrink:0,background:'var(--bg)',boxSizing:'border-box',width:'100%'}}>
        {/* Search */}
        <div style={{flex:'1 1 0',display:'flex',alignItems:'center',gap:7,background:'var(--surface2)',border:'1.5px solid var(--border)',borderRadius:12,padding:'0 10px',height:42,minWidth:0,overflow:'hidden'}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="পণ্য খুঁজুন..." style={{flex:1,background:'transparent',border:'none',outline:'none',color:'var(--text)',fontSize:'0.83rem',fontFamily:'var(--font-bn)',minWidth:0,width:'100%'}} autoFocus/>
        ...
        {/* Scan btn */}
        <button onClick={()=>setShowCam(true)} style={{width:42,height:42,minWidth:42,maxWidth:42,borderRadius:12,background:'linear-gradient(135deg,var(--primary),var(--accent))',border:'none',display:'flex',alignItems:'center',justifyContent:'center',color:'white',cursor:'pointer',flexShrink:0}}>
          <Camera size={17}/>
        </button>
        {/* Cart btn */}
        <button onClick={()=>setShowCart(true)} style={{position:'relative',width:42,height:42,minWidth:42,maxWidth:42,borderRadius:12,background:count>0?'linear-gradient(135deg,#1a73e8,#0d47a1)':'var(--surface3)',border:count>0?'none':'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'center',color:count>0?'white':'var(--text3)',cursor:'pointer',flexShrink:0}}>
          <ShoppingCart size={17}/>
          {count>0&&<span style={{position:'absolute',top:-5,right:-5,width:18,height:18,background:'var(--danger)',borderRadius:'50%',color:'white',fontSize:'0.58rem',fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',border:'2px solid var(--bg)'}}>{count}</span>}
        </button>
      </div>

      {/* ── Product Grid ── */}
      <div style={{flex:1,overflowY:'scroll',WebkitOverflowScrolling:'touch',overscrollBehavior:'contain',padding:'4px 10px 100px'}}>
        {filtered.length===0?(
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {filtered.map(item=>{
              ...
              return (
                <button key={item.id} onClick={()=>!isOut&&addItem(item)} disabled={isOut}
      ...
      {/* ── Floating cart bar ── */}
      {count>0&&!showCart&&(
        <div onClick={()=>setShowCart(true)} style={{position:'fixed',bottom:'calc(var(--nav-h) + env(safe-area-inset-bottom) + 12px)',left:'50%',transform:'translateX(-50%)',width:'calc(100% - 32px)',maxWidth:380,background:'linear-gradient(135deg,var(--primary),var(--accent))',borderRadius:16,padding:'13px 16px',boxShadow:'0 8px 24px rgba(255,87,34,0.45)',display:'flex',alignItems:'center',justifyContent:'space-between',zIndex:140,cursor:'pointer'}}>
          <div style={{color:'white'}}>
            <div style={{fontSize:'0.68rem',opacity:0.85,fontFamily:'var(--font-bn)'}}>কার্টে {count} পণ্য</div>
            <div style={{fontFamily:'var(--font-mono)',fontWeight:800,fontSize:'1.1rem'}}>{fmt(total)}</div>
          </div>
          <span style={{fontSize:'0.82rem',fontWeight:600,fontFamily:'var(--font-bn)',color:'white'}}>চেকআউট →</span>
        </div>
      )}
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div>
                  <div className="modal-title">🛒 কার্ট ({count} পণ্য)</div>
                  <div style={{fontSize:'0.7rem',color:'var(--text3)',fontFamily:'var(--font-bn)',marginTop:2}}>পরিমাণ ও মূল্য পরিবর্তন করতে পারেন</div>
                </div>
                ...
                <>
                    ...
                    <div key={item.product.id} style={{padding:'10px 0',borderBottom:'1px solid var(--border)'}}>
                      ...
                      <div style={{display:'flex',gap:8,alignItems:'center'}}>
                        <div style={{display:'flex',alignItems:'center',gap:5,background:'var(--surface2)',borderRadius:10,padding:'4px 6px',border:'1px solid var(--border)',flexShrink:0}}>
                          <button onClick={()=>qty(item.product.id,-1)} disabled={item.qty<=1} style={{width:26,height:26,borderRadius:7,border:'none',background:'var(--surface3)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text2)'}}><Minus size={12}/></button>
                          <span style={{minWidth:24,textAlign:'center',fontFamily:'var(--font-mono)',fontWeight:700,fontSize:'0.88rem'}}>{item.qty}</span>
                          <button onClick={()=>qty(item.product.id,1)} style={{width:26,height:26,borderRadius:7,border:'none',background:'var(--primary)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'white'}}><Plus size={12}/></button>
                        </div>
                        <input className="input" type="number" value={item.unitPrice} onChange={e=>price(item.product.id,e.target.value)} style={{flex:1,padding:'6px 10px',fontSize:'0.82rem',height:36,minWidth:0}} step="any"/>
                  ...
                  <div style={{background:'var(--surface2)',borderRadius:12,padding:'12px 14px',marginTop:14,marginBottom:12}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
                      ...
                      <span style={{fontFamily:'var(--font-mono)',fontWeight:800,fontSize:'1.1rem'}}>{fmt(total)}</span>
                    </div>
                  ...
                  {payMode==='cash'&&(
                    <div className="input-group">
                      ...
                      <input className="input" type="number" placeholder={String(Math.ceil(total))} value={cash} onChange={e=>setCash(e.target.value)} step="any"/>
                      {cashN>0&&(
                  ...
                  <button className="btn btn-primary btn-full" onClick={checkout} disabled={processing} style={{padding:14,fontSize:'0.9rem',marginBottom:4}}>
                    {processing
                      ...
                      :payMode==='cash'?`✅ বিক্রয় নিশ্চিত — ${fmt(total)}`:`📋 বকেয়া রেকর্ড`
                    }
                  </button>
                </>
              )}
      ...
      {showOk&&lastSale&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:900,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
          <div style={{background:'var(--surface)',borderRadius:24,padding:28,width:'100%',maxWidth:360,textAlign:'center',animation:'scale-in 0.35s cubic-bezier(0.34,1.56,0.64,1)'}}>
            ...
            <button className="btn btn-primary btn-full" onClick={()=>{setShowOk(false);setLastSale(null)}} style={{padding:13}}>নতুন বিক্রয় শুরু করুন</button>
          </div>
        </div>
      )}
