      ...
      videoRef.current.onloadedmetadata = async () => {
        ...
        if ('BarcodeDetector' in window) {
          try {
            ...
            detectorRef.current = new BDClass({ formats: ['qr_code','code_128','ean_13','ean_8','code_39','code_93','upc_a','upc_e','itf','codabar','data_matrix','aztec'] })
            startDetectionLoop()
    ...
    <div style={{ position:'fixed', inset:0, zIndex:900, background:'#000', display:'flex', flexDirection:'column' }}>
      <div style={{ flex:1, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', left:0, right:0, top:'calc(50% + 145px)', textAlign:'center' }}>
          <span style={{ background:'rgba(0,0,0,0.65)', borderRadius:20, padding:'6px 16px', color:'rgba(255,255,255,0.9)', fontFamily:'var(--font-bn)', fontSize:'0.78rem', display:'inline-block' }}>
            {!camReady ? '📷 ক্যামেরা চালু হচ্ছে...' : noDetector ? '⌨️ নিচে লিখুন' : '🔍 বারকোড ফ্রেমে ধরুন — সরাসরি কার্টে যাবে'}
          </span>
        </div>
      </div>
      <div style={{ background:'rgba(13,13,22,0.98)', padding:'14px 16px', paddingBottom:'max(20px,env(safe-area-inset-bottom))', borderTop:'1px solid rgba(255,255,255,0.08)' }}>
        ...
        <div style={{ fontSize:'0.68rem', color:'rgba(255,255,255,0.4)', fontFamily:'var(--font-bn)', marginBottom:8, textAlign:'center' }}>
          ম্যানুয়ালি বারকোড / পণ্যের নাম লিখুন
        </div>
...
export default function POSSection() {
  ...
  // ── Barcode scan handler — directly add to cart (Issue #7) ──
  function handleBarcodeScan(code: string) {
    ...
    if (found) {
      if (found.quantity <= 0) {
        toast(`⚠ "${found.name}" স্টক শেষ`, 'er')
        return
      }
      // Directly add to cart — no popup
    ...
    <div style={{
      ...
      <div style={{ display:'flex', alignItems:'center', gap:7, padding:'8px 12px 6px', flexShrink:0, background:'var(--bg)', boxSizing:'border-box', width:'100%', overflow:'hidden' }}>
        ...
        <div style={{ flex:'1 1 0', display:'flex', alignItems:'center', gap:7, background:'var(--surface2)', border:'1.5px solid var(--border)', borderRadius:12, padding:'0 10px', height:42, minWidth:0, overflow:'hidden' }}>
          ...
          />
          {search && (
            <button onClick={()=>setSearch('')} style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', padding:0, flexShrink:0, lineHeight:1 }}>✕</button>
          )}
        </div>
