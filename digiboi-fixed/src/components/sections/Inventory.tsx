...
function QRCodeSVG({ value, size = 140 }: { value: string; size?: number }) {
  ...
  const cells = 21
  const cs = size / cells
  const M = Array.from({ length: cells }, (_, r) =>
  ...
  return (
    <div style={{ display:'inline-flex', flexDirection:'column', alignItems:'center', background:'white', padding:10, borderRadius:12, border:'1.5px solid var(--border)' }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {M.map((row,r)=>row.map((cell,c)=>cell?<rect key={`${r}-${c}`} x={c*cs} y={r*cs} width={cs} height={cs} fill="#000"/>:null))}
      </svg>
      <div style={{ fontSize:'0.58rem', fontFamily:'monospace', color:'#333', marginTop:4, wordBreak:'break-all', textAlign:'center', maxWidth:size, userSelect:'all' }}>{value}</div>
    </div>
  )
}
...
function BarcodeDisplay({ value }: { value: string }) {
  if (!value) return null
  return (
    <div style={{ textAlign:'center', padding:'10px 12px', background:'white', borderRadius:10, border:'1px solid var(--border)', width:'100%' }}>
      <div style={{ display:'flex', justifyContent:'center', gap:1, height:44, marginBottom:4 }}>
      ...
      <div style={{ fontSize:'0.58rem', fontFamily:'monospace', color:'#333', userSelect:'all', letterSpacing:'0.06em' }}>{value}</div>
    </div>
  )
}
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-header">
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:4 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <QrCode size={17} color="var(--primary)"/>
              <span style={{ fontWeight:700, fontSize:'0.92rem', fontFamily:'var(--font-bn)', color:'var(--text)' }}>পণ্যের QR তথ্য</span>
            </div>
        ...
        <div className="modal-body">
          <div style={{ display:'flex', justifyContent:'center', marginBottom:14 }}>
            <QRCodeSVG value={item.product_link||item.id} size={140}/>
          </div>
          <div style={{ background:'var(--surface2)', borderRadius:14, padding:12, display:'flex', flexDirection:'column', gap:10, marginBottom:12 }}>
            <div style={{ display:'flex', gap:10, alignItems:'center' }}>
              <div style={{ width:40, height:40, borderRadius:10, background:'var(--primary-bg)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.2rem', flexShrink:0 }}>
                {CAT_EMOJI[item.category]||'📦'}
              </div>
              <div>
                <div style={{ fontWeight:700, fontSize:'0.92rem', color:'var(--text)', fontFamily:'var(--font-bn)' }}>{item.name}</div>
                <div style={{ fontSize:'0.7rem', color:'var(--text3)', fontFamily:'var(--font-bn)', marginTop:1 }}>📂 {item.category}</div>
              </div>
            </div>
            {item.notes && (
              <div style={{ background:'var(--surface)', borderRadius:8, padding:'8px 10px', fontSize:'0.75rem', color:'var(--text2)', fontFamily:'var(--font-bn)', lineHeight:1.5 }}>📝 {item.notes}</div>
            )}
            ...
            <div style={{ display:'flex', alignItems:'flex-start', gap:9, background:'linear-gradient(135deg,var(--primary-bg),rgba(255,152,0,0.07))', border:'1.5px solid var(--primary-glow)', borderRadius:10, padding:'10px 12px' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:'0.65rem', color:'var(--text3)', fontFamily:'var(--font-bn)', marginBottom:3 }}>এই পণ্যটি Digiboi ডিজিটাল সিস্টেমে নিবন্ধিত</div>
                <div style={{ fontFamily:'monospace', fontSize:'0.62rem', color:'var(--text2)', background:'var(--surface)', borderRadius:5, padding:'3px 7px', wordBreak:'break-all', userSelect:'all' }}>🔑 {item.product_link||item.id}</div>
              </div>
            </div>
          </div>
          <div style={{ marginBottom:4 }}>
            <div style={{ fontSize:'0.7rem', color:'var(--text3)', fontFamily:'var(--font-bn)', marginBottom:6, textAlign:'center' }}>বারকোড</div>
        ...
        <div key={item.id} className="product-card anim-fade-up" style={{ animationDelay:`${idx*0.03}s` }}>
          ...
          <div className="product-meta">
            ...
            <div style={{ display:'flex', gap:5, marginTop:6, flexWrap:'wrap', justifyContent:'flex-end' }}>
              {item.product_link&&<button className="btn btn-xs btn-ghost" style={{ padding:'4px 6px' }} onClick={()=>{setBarcodeItem(item);setModal('barcode')}} title="QR দেখুন"><QrCode size={12}/></button>}
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-header">
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:8 }}>
                ...
                <button onClick={()=>setModal(null)} style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', padding:4, borderRadius:8, flexShrink:0 }}><X size={20}/></button>
              </div>
            </div>
            ...
            <div className="modal-body">
              ...
              {profitPerUnit!==null&&(
                <div style={{ background:profitPerUnit>=0?'var(--success-light)':'var(--danger-light)', borderRadius:10, padding:'10px 14px', marginBottom:14, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                  <div style={{ textAlign:'center' }}>
                    <div style={{ fontFamily:'var(--font-mono)', fontWeight:700, fontSize:'0.82rem', color:profitPerUnit>=0?'var(--success)':'var(--danger)' }}>{profitPerUnit>=0?'+':''}{fmt(profitPerUnit)}</div>
                    <div style={{ fontSize:'0.58rem', color:'var(--text3)', fontFamily:'var(--font-bn)' }}>লাভ/একক</div>
                  </div>
                  <div style={{ textAlign:'center' }}>
                    <div style={{ fontFamily:'var(--font-mono)', fontWeight:700, fontSize:'0.82rem', color:profitPerUnit>=0?'var(--success)':'var(--danger)' }}>{profitMargin}%</div>
                    <div style={{ fontSize:'0.58rem', color:'var(--text3)', fontFamily:'var(--font-bn)' }}>মার্জিন</div>
                  </div>
                  {totalValue!==null&&(
                    <div style={{ textAlign:'center' }}>
                      <div style={{ fontFamily:'var(--font-mono)', fontWeight:700, fontSize:'0.82rem', color:'var(--text)' }}>{fmt(totalValue)}</div>
              ...
              <div className="input-group">
                ...
                <input className="input" placeholder="স্ক্যান করুন বা লিখুন" value={form.barcode}
                ...
                {form.barcode&&(
                  <div style={{ marginTop:8, display:'flex', flexDirection:'column', gap:8, alignItems:'center' }}>
                    <QRCodeSVG value={form.barcode} size={110}/>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" style={{ flex:2 }} onClick={handleSave} disabled={saving}>
                {saving
                  ? <span style={{ display:'inline-flex', alignItems:'center', gap:8 }}><span className="spin" style={{ width:15,height:15,border:'2px solid rgba(255,255,255,0.4)',borderTopColor:'white',borderRadius:'50%',display:'inline-block' }}/>সংরক্ষণ...</span>
