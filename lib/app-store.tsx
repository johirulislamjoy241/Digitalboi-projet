...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    } catch { applyThemeToDOM('light') }
  }, [])

  const setTheme = useCallback((t: 'dark' | 'light') => {
    setThemeState(t); applyThemeToDOM(t)
    try { localStorage.setItem('db_theme', t) } catch { /* ignore */ }
  }, [])
  const toggleSidebar = useCallback(() => setSidebarOpen(v => !v), [])
  const setLang = useCallback((l: Lang) => { setLangState(l); try { localStorage.setItem('db_lang', l) } catch { /* ignore */ } }, [])
  const setCurrency = useCallback((c: string) => { setCurrencyState(c); try { localStorage.setItem('db_currency', c) } catch { /* ignore */ } }, [])
  const setLowStockThreshold = useCallback((n: number) => { setLSTState(n); try { localStorage.setItem('db_threshold', String(n)) } catch { /* ignore */ } }, [])
