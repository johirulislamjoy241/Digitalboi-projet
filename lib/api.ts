...
export function useApi() {
  ...
  async function addProduct(data: Partial<InventoryItem>): Promise<InventoryItem> {
    const res = await fetch('/api/inventory', { method: 'POST', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d.data
  }
  async function updateProduct(id: string, data: Partial<InventoryItem>): Promise<InventoryItem> {
    const res = await fetch(`/api/inventory/${id}`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d.data
  }
  async function deleteProduct(id: string): Promise<void> {
    const res = await fetch(`/api/inventory/${id}`, { method: 'DELETE', headers: authHeaders() })
    if (!res.ok) { const d = await res.json(); throw new Error(d.error) }
  }
  async function getTransactions(limit = 100, offset = 0): Promise<{ data: Transaction[]; count: number }> {
    const res = await fetch(`/api/transactions?limit=${limit}&offset=${offset}`, { headers: authHeaders() })
    const d = await res.json()
  ...
  async function doTransaction(data: {
    ...
    const res = await fetch('/api/transactions', { method: 'POST', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d
  }
  ...
  async function addDueEntry(data: Partial<DueLedgerEntry>): Promise<DueLedgerEntry> {
    const res = await fetch('/api/due-ledger', { method: 'POST', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d.data
  }
  async function updateDueEntry(id: string, data: Partial<DueLedgerEntry>): Promise<DueLedgerEntry> {
    const res = await fetch(`/api/due-ledger/${id}`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d.data
  }
  async function deleteDueEntry(id: string): Promise<void> {
    const res = await fetch(`/api/due-ledger/${id}`, { method: 'DELETE', headers: authHeaders() })
    if (!res.ok) { const d = await res.json(); throw new Error(d.error) }
  }
  ...
  async function addBuyer(data: Partial<Buyer>): Promise<Buyer> {
    const res = await fetch('/api/buyers', { method: 'POST', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d.data
  }
  async function updateBuyer(id: string, data: Partial<Buyer>): Promise<Buyer> {
    const res = await fetch(`/api/buyers/${id}`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify(data) })
    const d = await res.json()
    if (!res.ok) throw new Error(d.error)
    return d.data
  }
  async function deleteBuyer(id: string): Promise<void> {
    const res = await fetch(`/api/buyers/${id}`, { method: 'DELETE', headers: authHeaders() })
    if (!res.ok) { const d = await res.json(); throw new Error(d.error) }
  }
  return {
