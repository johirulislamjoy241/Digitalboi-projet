...
export async function GET(req: NextRequest) {
  ...
  const { data, error, count } = await supabase.from('transactions').select('*', { count: 'exact' }).eq('user_id', uid).order('date', { ascending: false }).range(offset, offset + limit - 1)
...
export async function POST(req: NextRequest) {
  ...
  const { product_id, product_name, txn_type, quantity, unit, buy_price, sell_price, notes, new_buy_price, new_sell_price, low_stock_threshold } = body
  if (!product_id || !product_name || !txn_type || !quantity) return NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
  const supabase = createServiceRoleClient()
  const qty = Number(quantity)
  const threshold = Number(low_stock_threshold) || 10
  const { data: product, error: pErr } = await supabase.from('inventory').select('*').eq('id', product_id).eq('user_id', uid).single()
  ...
  if (txn_type === 'in') { new_qty = old_qty + qty; type = 'Stock In' }
  else if (txn_type === 'out') {
    if (qty > old_qty) return NextResponse.json({ error: `Not enough stock. Available: ${old_qty}` }, { status: 400 })
    new_qty = old_qty - qty; profit_loss = (sp - bp) * qty; type = profit_loss < 0 ? 'Loss' : 'Stock Out'
  } else if (txn_type === 'price') { type = 'Price Update' }
  ...
  const { error: updateErr } = await supabase.from('inventory').update(updateFields).eq('id', product_id).eq('user_id', uid)
  if (updateErr) return NextResponse.json({ error: updateErr.message }, { status: 500 })
  const { data: txn, error: txnErr } = await supabase.from('transactions').insert({
    user_id: uid, product_id, product_name, type, txn_type, quantity: qty,
    unit: unit || product.unit || 'pcs', buy_price: bp, sell_price: sp,
    profit_loss, old_qty, new_qty, notes, date: new Date().toISOString(),
  }).select('*').single()
