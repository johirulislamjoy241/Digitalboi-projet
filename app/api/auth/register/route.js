...
export async function POST(req) {
  try {
    ...
    if (recoveryCode && (recoveryCode.length!==6||!/^\d{6}$/.test(recoveryCode)))
      return NextResponse.json({success:false,error:"রিকভারি কোড অবশ্যই ৬ সংখ্যার হতে হবে"},{status:400});

    // Duplicate checks
    const { data:existing } = await supabaseAdmin.from("users").select("id").eq("phone",normalized).maybeSingle();
    if (existing) return NextResponse.json({success:false,error:"এই মোবাইল নম্বরে ইতিমধ্যে অ্যাকাউন্ট আছে"},{status:409});
    if (email) {
    ...
    const recoveryHash = recoveryCode ? await bcrypt.hash(recoveryCode, 10) : null;
    ...
    const { data:user, error:userErr } = await supabaseAdmin.from("users").insert({
      ...
      recovery_code_set_at: recoveryCode ? new Date().toISOString() : null,
    ...
    const { data:shop, error:shopErr } = await supabaseAdmin.from("shops").insert({
      ...
      nid, division, district, upazila, post_code:postcode, address,
      latitude:lat, longitude:lng, plan:"Free", status:"active", is_active:true
