...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // 'remaining' is a GENERATED ALWAYS column — never write it
  delete body.remaining
  if (body.paid_amount !== undefined || body.total_amount !== undefined) {
    const supabase = createServiceRoleClient()
    const { data: current } = await supabase.from('due_ledger').select('total_amount, paid_amount').eq('id', params.id).eq('user_id', uid).single()
    if (current) {
  ...
  const supabase = createServiceRoleClient()
  const { data, error } = await supabase.from('due_ledger').update(body).eq('id', params.id).eq('user_id', uid).select('*').single()
