...
export async function POST(req) {
  try {
    const { shopId, name, brand, categoryId, sellPrice, buyPrice, stock, lowStockAlert, unit, barcode } = await req.json();
    ...
    const { data, error } = await supabaseAdmin.from("products").insert({
      ...
      qr_code: `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=DIGIBOI-${bc}`,
      is_active: true
