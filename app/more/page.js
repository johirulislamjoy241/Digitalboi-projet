...
function ReportsPage({ onBack, user }) {
  ...
  const load = (p) => {
    ...
    setLoading(true);
    const { from, to } = getRange(p || period);
    const token = typeof window !== "undefined" ? localStorage.getItem("digiboi_token") : null;
    fetch(`/api/reports?from=${from}&to=${to}`, { headers: token ? { Authorization:`Bearer ${token}` } : {} })
        ...
        <div style={{ padding:16 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16 }}>
            {[
              ...
              { l:"মোট লাভ",     v:taka(data?.totalProfit ||0), c:data?.totalProfit>=0?T.success:T.danger },
              ...
              <div style={{ display:"flex",gap:6,alignItems:"flex-end",height:90 }}>
                {data.monthlyChart.map((d,i) => (
                  <div key={i} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
                    <div style={{ fontSize:8,color:T.brand,fontWeight:700 }}>{taka(d.value).replace("৳","")}</div>
                    <div style={{ width:"100%",background:T.brandGrad,borderRadius:"4px 4px 0 0",height:Math.max(6,Math.round((d.value/maxChart)*72)),transition:"height 0.4s" }}/>
          ...
          {data?.topProducts?.length > 0 && (
            <Card style={{ marginBottom:16 }}>
              <div style={{ fontWeight:800,fontSize:14,marginBottom:12 }}>🏆 সেরা পণ্য</div>
          ...
          {data?.lowStock?.length > 0 && (
            <Card style={{ marginBottom:16,border:`1.5px solid ${T.warning}40` }}>
              ...
              {data.lowStock.slice(0,4).map((p,i) => (
                <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${T.border}` }}>
                  <span style={{ fontSize:13,fontWeight:600 }}>{p.name}</span>
                  <span style={{ fontSize:12,fontWeight:700,color:T.warning,background:`${T.warning}10`,padding:"3px 8px",borderRadius:6 }}>{p.stock} বাকি</span>
                </div>
              ))}
            </Card>
          )}
...
function SalesHistoryPage({ onBack, user }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/sales?shopId=${user.shopId}&limit=100`)
      .then(r => r.json()).then(d => { setSales(d.success ? (d.data||[]) : []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [user?.shopId]);

  const filtered = sales.filter(s =>
    (filter === "all" || s.status === filter) &&
    (!search || s.invoice_id?.includes(search) || s.customers?.name?.includes(search))
  );
  ...
  const collectDue = async () => {
    ...
    setSaving(true);
    await fetch(`/api/sales/${collectFor.id}`, {
      method:"PATCH", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ action:"collect_due", amount:+colAmt })
    });
    setSaving(false); setCollectFor(null); setColAmt("");
    fetch(`/api/sales?shopId=${user.shopId}&limit=100`).then(r=>r.json()).then(d=>setSales(d.data||[]));
  };
  ...
  return (
    <div style={{ paddingBottom:80 }}>
      ...
      <div style={{ padding:"12px 16px 0" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12 }}>
          {[
            ...
            { l:"মোট বাকি",   v:taka(totalDue),   c:T.warning },
          ].map(s => (
        ...
        {/* সার্চ */}
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 ইনভয়েস বা গ্রাহক..."
        ...
        <div style={{ display:"flex",gap:6,marginBottom:12 }}>
          {[["all","সব"],["paid","পরিশোধ"],["due","বাকি"],["partial","আংশিক"]].map(([k,l]) => (
            <button key={k} onClick={() => setFilter(k)}
              style={{ padding:"6px 10px",border:`2px solid ${filter===k?T.brand:T.border}`,borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",background:filter===k?`${T.brand}12`:T.surface,color:filter===k?T.brand:T.textMuted,fontFamily:"inherit" }}>
              {l}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding:"0 16px" }}>
        {loading ? <div style={{ padding:40,textAlign:"center" }}><Spinner/></div>
        : filtered.length === 0 ? <div style={{ padding:"40px 0",textAlign:"center",color:T.textMuted }}>কোনো বিক্রয় নেই</div>
        : filtered.map(s => (
          <Card key={s.id} style={{ marginBottom:10,padding:"14px 16px" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6 }}>
              <div>
                ...
                <div style={{ fontSize:11,color:T.textMuted,marginTop:1 }}>{s.invoice_id} · {s.sale_items?.length||0}টি পণ্য</div>
              </div>
              <div style={{ textAlign:"right" }}>
                <div style={{ fontWeight:800,fontSize:16,color:T.brand }}>{taka(s.total)}</div>
                <span style={{ fontSize:11,fontWeight:700,color:STATUS_COLOR[s.status]||T.info }}>{STATUS_LABEL[s.status]||s.status}</span>
              </div>
            </div>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              <span style={{ fontSize:11,color:T.textMuted,background:T.bg,padding:"3px 8px",borderRadius:6 }}>{s.payment_method}</span>
              {(+s.due)>0 && (
      ...
      {collectFor && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={() => setCollectFor(null)}>
          <div style={{ background:T.surface,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,padding:20 }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ fontSize:13,color:T.textMuted,marginBottom:14 }}>মোট বাকি: {taka(collectFor.due)}</div>
...
export default function MorePage({ user, setActive, onLogout }) {
  ...
  const MENU = [
    { id:"sales",   icon:"🧾", label:"বিক্রয় ইতিহাস",       desc:"সব বিক্রয়, বাকি, রিসিপ্ট" },
    { id:"reports", icon:"📊", label:"রিপোর্ট ও বিশ্লেষণ",  desc:"মোট লাভ, আর্থিক সারসংক্ষেপ" },
