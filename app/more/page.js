...
function ExpensesPage({ user, onBack }) {
  ...
  const [form,      setForm]      = useState({ category:"অন্যান্য", amount:"", note:"", expenseDate:"" });
  const [saving,    setSaving]    = useState(false);
  const [toast,     setToast]     = useState(null);
  const shopId = user?.shopId;
  const today  = new Date().toISOString().slice(0,10);

  const cats = ["ভাড়া","বেতন","বিদ্যুৎ","পানি","ইন্টারনেট","পরিবহন","মার্কেটিং","মেরামত","ক্রয়","অন্যান্য"];
  ...
  const save = async () => {
    ...
    if (!amt||amt<=0) return setToast({msg:"পরিমাণ সঠিকভাবে দিন",type:"error"});
    setSaving(true);
    try {
      const res = await fetch("/api/expenses", { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ shopId, category:form.category, amount:amt, note:form.note||null, expenseDate:form.expenseDate||today }) });
      const d = await res.json();
      if (d.success) {
        setExpenses(prev=>[d.data,...prev]);
        setForm({category:"অন্যান্য",amount:"",note:"",expenseDate:""});
        setShowAdd(false);
  ...
  const del = async (id) => {
    ...
    try {
      ...
      setSelected(null);
      setToast({msg:"মুছে ফেলা হয়েছে",type:"success"});
    } catch { setToast({msg:"মুছতে ব্যর্থ",type:"error"}); }
  };

  const total   = expenses.reduce((s,e)=>s+(+e.amount||0),0);
  const byMonth = expenses.reduce((acc,e)=>{
    const m = (e.expense_date||e.created_at||"").slice(0,7);
    acc[m]=(acc[m]||0)+(+e.amount||0); return acc;
  },{});
  ...
  return (
    <div style={{paddingBottom:80}}>
      ...
      {/* Detail modal */}
      {selected && (
        <div onClick={()=>setSelected(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"flex-end",flexDirection:"column"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,padding:20}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <h3 style={{margin:0,fontWeight:800}}>খরচ বিস্তারিত</h3>
              <button onClick={()=>setSelected(null)} style={{background:`${T.danger}15`,border:"none",borderRadius:8,padding:8,cursor:"pointer",color:T.danger}}>✕</button>
            </div>
            <div style={{background:T.bg,borderRadius:T.radiusSm,padding:"14px 16px",marginBottom:14}}>
              {[
                {l:"ক্যাটাগরি",v:selected.category},
                {l:"পরিমাণ",   v:taka(selected.amount)},
                {l:"তারিখ",    v:selected.expense_date||fmtDate(selected.created_at)},
                {l:"নোট",      v:selected.note},
            ...
            <Btn variant="danger" full onClick={()=>del(selected.id)}>🗑️ মুছে ফেলুন</Btn>
          </div>
        </div>
      )}
      ...
      <div style={{padding:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          <Card style={{textAlign:"center",padding:"14px 10px"}}>
            ...
            <div style={{fontSize:11,color:T.textMuted}}>মোট খরচ</div>
          </Card>
          <Card style={{textAlign:"center",padding:"14px 10px"}}>
            <div style={{fontSize:18,fontWeight:800,color:T.warning}}>{expenses.length}</div>
            <div style={{fontSize:11,color:T.textMuted}}>মোট এন্ট্রি</div>
          </Card>
        </div>

        <Btn variant="primary" full onClick={()=>setShowAdd(v=>!v)} style={{marginBottom:14}}>
          {showAdd?"বাতিল":"+ নতুন খরচ যোগ করুন"}
        </Btn>
        ...
        {showAdd && (
          <Card style={{marginBottom:14,border:`1.5px solid ${T.brand}30`}}>
            ...
            <Input label="পরিমাণ ৳ *" type="number" value={form.amount} onChange={v=>setForm(f=>({...f,amount:v}))} placeholder="0.00" inputMode="decimal"/>
            <Input label="তারিখ" type="date" value={form.expenseDate||today} onChange={v=>setForm(f=>({...f,expenseDate:v}))}/>
        ...
        {loading ? <Spinner/> : !expenses.length
          ? <EmptyState icon="💸" title="কোনো খরচ নেই" sub="খরচ যোগ করুন"/>
          : expenses.map(e=>(
            <Card key={e.id} onClick={()=>setSelected(e)} style={{marginBottom:8,padding:"12px 14px",cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <div style={{fontSize:11,color:T.textMuted,marginTop:2}}>
                    📅 {e.expense_date||fmtDate(e.created_at)}
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{fontWeight:800,fontSize:15,color:T.danger}}>{taka(e.amount)}</div>
                  <button onClick={ev=>{ev.stopPropagation();del(e.id);}} style={{background:`${T.danger}15`,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",color:T.danger,fontSize:12}}>✕</button>
                </div>
              </div>
            </Card>
          ))
...
function SuppliersPage({ user, onBack }) {
  ...
  const [form,      setForm]      = useState({ name:"", phone:"", email:"", address:"", notes:"", joinDate:"" });
  const [saving,    setSaving]    = useState(false);
  const [toast,     setToast]     = useState(null);
  const shopId = user?.shopId;
  const today  = new Date().toISOString().slice(0,10);
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/suppliers", { method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ shopId, name:form.name, phone:form.phone||null, email:form.email||null, address:form.address||null, notes:form.notes||null }) });
      const d = await res.json();
      if (d.success) {
        setSuppliers(prev=>[d.data,...prev]);
        setForm({name:"",phone:"",email:"",address:"",notes:"",joinDate:""});
        setShowAdd(false);
  ...
  const getSupplierProducts = (sid) => products.filter(p => p.supplier_id === sid);
  const totalValue = (prods) => prods.reduce((s,p)=>(s+(+p.buy_price||0)*(+p.stock||0)),0);
  ...
  return (
    <div style={{paddingBottom:80}}>
      ...
      {selected && (
        <div onClick={()=>setSelected(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",padding:"0 0 20px"}}>
            <div style={{display:"flex",justifyContent:"center",padding:"10px 0 4px"}}>
              <div style={{width:40,height:4,borderRadius:2,background:T.border}}/>
            </div>
            <div style={{background:T.brandGrad,margin:"0 0 16px",padding:"16px 20px"}}>
              <div style={{color:"#fff",fontWeight:800,fontSize:18}}>{selected.name}</div>
              {selected.phone && <div style={{color:"rgba(255,255,255,0.85)",fontSize:13}}>📞 {selected.phone}</div>}
              {selected.address && <div style={{color:"rgba(255,255,255,0.75)",fontSize:12}}>📍 {selected.address}</div>}
            </div>
            <div style={{padding:"0 20px"}}>
              {/* Stats */}
              {(() => { const sp = getSupplierProducts(selected.id); const tv = totalValue(sp); return (
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
                  <div style={{background:`${T.brand}10`,borderRadius:T.radiusSm,padding:"12px",textAlign:"center",border:`1px solid ${T.brand}20`}}>
                    <div style={{fontWeight:800,fontSize:16,color:T.brand}}>{sp.length}</div>
                    <div style={{fontSize:11,color:T.textMuted}}>মোট পণ্য</div>
                  </div>
                  <div style={{background:`${T.success}10`,borderRadius:T.radiusSm,padding:"12px",textAlign:"center",border:`1px solid ${T.success}20`}}>
                    <div style={{fontWeight:800,fontSize:14,color:T.success}}>{taka(tv)}</div>
                    <div style={{fontSize:11,color:T.textMuted}}>মোট স্টক মূল্য</div>
                  </div>
                </div>
              ); })()}

              {/* Products list */}
              <div style={{fontWeight:700,fontSize:13,color:T.textSub,marginBottom:8}}>📦 সরবরাহকৃত পণ্য</div>
              {getSupplierProducts(selected.id).length===0
                ? <div style={{color:T.textMuted,fontSize:13,padding:"12px 0"}}>এই সরবরাহকারীর কোনো পণ্য নেই</div>
                : getSupplierProducts(selected.id).map(p=>(
                  <div key={p.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${T.border}`,fontSize:13}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:T.textMuted}}>স্টক: {p.stock} {p.unit}</div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontWeight:700,color:T.brand}}>{taka(p.sell_price)}</div>
                      <div style={{fontSize:11,color:T.textMuted}}>ক্রয়: {taka(p.buy_price)}</div>
                    </div>
                  </div>
                ))
              }
              ...
              {selected.notes && (
                <div style={{marginTop:14,background:T.bg,borderRadius:T.radiusSm,padding:"10px 14px"}}>
                  <div style={{fontSize:12,color:T.textMuted,marginBottom:4}}>নোট</div>
      ...
      <div style={{padding:16}}>
        ...
        {showAdd && (
          <Card style={{marginBottom:14,border:`1.5px solid ${T.brand}30`}}>
            ...
            <Input label="ঠিকানা" value={form.address} onChange={v=>setForm(f=>({...f,address:v}))} placeholder="ঠিকানা"/>
            <Input label="নোট (ঐচ্ছিক)" value={form.notes} onChange={v=>setForm(f=>({...f,notes:v}))} placeholder="বিশেষ তথ্য"/>
            <div style={{display:"flex",gap:8}}>
              <Btn variant="secondary" full onClick={()=>setShowAdd(false)}>বাতিল</Btn>
              <Btn variant="primary" full onClick={save} disabled={saving}>{saving?"সংরক্ষণ...":"✅ যোগ করুন"}</Btn>
            </div>
        ...
        {loading ? <Spinner/> : !suppliers.length
          ? <EmptyState icon="🚚" title="কোনো সরবরাহকারী নেই" sub="প্রথম সরবরাহকারী যোগ করুন"/>
          : suppliers.map(s=>{
            const sp = getSupplierProducts(s.id);
            return (
              <Card key={s.id} onClick={()=>setSelected(s)} style={{marginBottom:10,padding:14,cursor:"pointer"}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  <div style={{width:46,height:46,borderRadius:12,background:T.brandGrad,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:18,flexShrink:0}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:T.brand,marginTop:4,fontWeight:600}}>
                      📦 {sp.length}টি পণ্য · মূল্য: {taka(totalValue(sp))}
                    </div>
                  </div>
                  {+s.due_amount>0 && <Badge color="warning">{taka(s.due_amount)} বাকি</Badge>}
                </div>
              </Card>
            );
          })
