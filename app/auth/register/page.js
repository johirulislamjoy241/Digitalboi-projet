...
export default function RegisterPage({ onBack, onComplete }) {
  ...
  useEffect(() => {
    ...
    // Don't load Google Maps script (no free API key) — show manual input instead
    // If you add an API key later, uncomment below:
  ...
  const submit = async () => {
    ...
    try {
      ...
      const fields = {
        ...
        lat:data.lat, lng:data.lng, password:data.password, recoveryCode:data.recoveryCode
      };
        ...
        {step===3 && (
          <>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <p style={{ fontSize:11, color:T.textMuted, marginBottom:8 }}>Google Maps থেকে আপনার দোকানের Latitude ও Longitude কপি করে দিন</p>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                <Input label="Latitude" value={data.lat} onChange={v => set("lat",v)} placeholder="23.8103" inputMode="decimal"/>
                <Input label="Longitude" value={data.lng} onChange={v => set("lng",v)} placeholder="90.4125" inputMode="decimal"/>
              </div>
              {data.lat && data.lng && (
                <div style={{ fontSize:11, color:T.success, fontWeight:600 }}>
                  ✅ লোকেশন: {data.lat}, {data.lng}
                </div>
              )}
            </div>
        ...
        {step===4 && (
          <>
            ...
            <div style={{ background:`${T.brand}08`, borderRadius:T.radiusSm, padding:"12px 14px", marginBottom:16, border:`1px solid ${T.brand}20` }}>
              ...
              <div style={{ fontSize:12, color:T.textSub }}>👤 {data.ownerName} ({data.ownerGender})</div>
              <div style={{ fontSize:12, color:T.textSub }}>📱 {data.phone}{data.email ? ` · ${data.email}` : ""}</div>
              {data.nid && <div style={{ fontSize:12, color:T.textSub }}>🪪 NID: {data.nid}</div>}
              <div style={{ fontSize:12, color:T.textSub }}>🏪 {data.bizName} — {data.bizCategory} ({data.bizType})</div>
              {data.bizPhone && <div style={{ fontSize:12, color:T.textSub }}>📞 দোকান: {data.bizPhone}</div>}
              {data.tradeLicense && <div style={{ fontSize:12, color:T.textSub }}>📄 লাইসেন্স: {data.tradeLicense}</div>}
              {data.division && <div style={{ fontSize:12, color:T.textSub }}>📍 {data.division}{data.district ? `, ${data.district}` : ""}{data.upazila ? `, ${data.upazila}` : ""}</div>}
              {data.address && <div style={{ fontSize:12, color:T.textSub }}>🏠 {data.address}</div>}
            </div>
            ...
            {/* Recovery Code */}
            <div style={{ background:`${T.info}08`,borderRadius:T.radiusSm,padding:"12px 14px",marginBottom:14,border:`1px solid ${T.info}20` }}>
              <div style={{ fontSize:12,fontWeight:800,color:T.info,marginBottom:6 }}>🔑 রিকভারি কোড (ঐচ্ছিক কিন্তু গুরুত্বপূর্ণ)</div>
              <p style={{ fontSize:12,color:T.textSub,marginBottom:10,margin:"0 0 10px" }}>পাসওয়ার্ড ভুলে গেলে এই ৬ সংখ্যার কোড দিয়ে পুনরুদ্ধার করতে পারবেন। মনে রাখুন ও নিরাপদ রাখুন।</p>
              <Input label="৬ সংখ্যার রিকভারি কোড" value={data.recoveryCode} onChange={v => set("recoveryCode",v.replace(/\D/g,"").slice(0,6))} placeholder="যেমন: 123456" inputMode="numeric"/>
            </div>
