...
function QRModal({ product, shopName, onClose }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div style={{ background:"#fff",borderRadius:T.radiusLg,width:"100%",maxWidth:320,boxShadow:T.shadowLg,overflow:"hidden" }}>
        {/* Header */}
        <div style={{ background:T.brandGrad,padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div style={{ color:"#fff" }}>
        ...
        {/* Details table */}
        <div style={{ margin:"0 16px",background:"#F9FAFB",borderRadius:T.radiusSm,padding:"10px 14px",marginBottom:16 }}>
          {[
            ...
            <div key={i} style={{ display:"flex",justifyContent:"space-between",fontSize:12,padding:"3px 0",borderBottom:i<4?`1px solid ${T.border}`:"none" }}>
              <span style={{ color:T.textMuted }}>{r.label}</span>
              <span style={{ fontWeight:600,color:T.text,maxWidth:"60%",textAlign:"right" }}>{r.value}</span>
            </div>
          ))}
        </div>
    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end" }}>
      <div style={{ background:T.surface,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:430,maxHeight:"90vh",overflow:"auto",padding:20 }}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <textarea value={form.description} onChange={e => set("description",e.target.value)} placeholder="পণ্যের বিস্তারিত বিবরণ, বৈশিষ্ট্য, ব্যবহারবিধি..." rows={3}
            style={{ width:"100%",padding:"9px 12px",border:`1.5px solid ${T.border}`,borderRadius:T.radiusSm,fontSize:13,fontFamily:"inherit",resize:"vertical",outline:"none",boxSizing:"border-box",color:T.text }}/> 
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
