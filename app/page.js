...
function BottomNav({ active, setActive }) {
  return (
    <nav style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:T.surface,borderTop:`1px solid ${T.border}`,display:"flex",zIndex:100,paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
      {NAV.map(item => {
...
function TopBar({ title, user }) {
  return (
    <div style={{ background:T.surface,padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.border}`,position:"sticky",top:0,zIndex:50 }}>
      <div>
      ...
      <div style={{ width:38,height:38,borderRadius:12,background:T.brandGrad,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:15,cursor:"pointer",flexShrink:0 }}>
...
export default function App() {
  ...
  // App screen
  const pages = {
    dashboard: <DashboardPage user={user} setActive={setActive}/>,
    pos:       <POSPage user={user}/>,
    inventory: <InventoryPage user={user}/>,
    customers: <CustomersPage user={user}/>,
    more:      <MorePage user={user} setActive={setActive} onLogout={handleLogout}/>,
  };

  return (
    <div style={{ background:T.bg,minHeight:"100dvh",display:"flex",justifyContent:"center" }}>
      <div style={{ width:"100%",maxWidth:430,minHeight:"100dvh",position:"relative" }}>
        {active !== "pos" && active !== "more" && <TopBar title={TITLES[active]||"Digiboi"} user={user}/>}
        <div style={{ paddingBottom:80 }}>{pages[active]}</div>
