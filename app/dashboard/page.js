...
export default function DashboardPage({ user, setActive }) {
  ...
  const maxProfit = Math.max(...(data?.chartData || []).map(d => Math.max(0, d.profit)), 1);
  ...
  return (
    <div style={{ padding: "16px 16px 0" }}>
      ...
      <div style={{ background: T.brandGrad, borderRadius: T.radiusLg, padding: "18px 20px", marginBottom: 16, color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <button onClick={load} style={{ position: "absolute", top: 12, right: 12, background: "rgba(255,255,255,0.2)", border: "none", borderRadius: 8, padding: "5px 10px", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          🔄 রিফ্রেশ
        </button>
      </div>
      ...
      <Card style={{ marginBottom: 16, background: `${T.brand}06`, border: `1px solid ${T.brand}20` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          {[
            ...
            { l: "পরিশোধ",    v: taka((data?.todayTotal || 0) - (data?.todayDue || 0)), c: T.success },
      ...
      {/* বাকি বিক্রির অ্যালার্ট */}
      {(data?.dueCount || 0) > 0 && (
        <Card style={{ marginBottom: 16, border: `1.5px solid ${T.warning}50`, background: `${T.warning}06` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontWeight: 800, fontSize: 14, color: T.warning }}>💰 {data.dueCount}টি বাকি বিক্রয়</div>
              <div style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>মোট বাকি: {taka(data.totalDue)}</div>
            </div>
            <button onClick={() => setActive("customers")} style={{ padding: "8px 14px", background: T.warning, border: "none", borderRadius: 10, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
              কালেক্ট করুন →
            </button>
          </div>
        </Card>
            ...
            {data.chartData.map((d, i) => (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                <div style={{ width: "100%", display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 1, height: 70 }}>
                  ...
                  <div style={{ width: "45%", background: T.success, borderRadius: "3px 3px 0 0", height: Math.max(2, Math.round((Math.max(0, d.profit) / maxProfit) * 60)), transition: "height 0.4s", opacity: 0.8 }} />
                </div>
      ...
      {data?.lowStockItems?.length > 0 && (
        <Card style={{ marginBottom: 16, border: `1.5px solid ${T.warning}40` }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: T.warning, marginBottom: 10 }}>
            ⚠️ লো স্টক ({data.lowStockItems.length}টি পণ্য)
          </div>
